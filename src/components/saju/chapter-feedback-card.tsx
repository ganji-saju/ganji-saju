...
export function ChapterFeedbackCard({
  ...
  if (state === 'auth-required') {
    return (
      <div
        className="mt-3 rounded-[12px] border border-[var(--app-line)] bg-white p-3 text-[14.4px] text-[var(--app-copy-soft)]"
        role="status"
      >
        피드백은{' '}
  ...
  return (
    ...
    >
      <div className="text-[14.4px] font-bold text-[var(--app-ink)]">
        이 풀이가 잘 맞았나요?
      </div>
      ...
      <div className="mt-2 flex items-center gap-1" role="radiogroup" aria-label="별점">
        {[1, 2, 3, 4, 5].map((value) => {
          ...
          return (
            <button
              ...
              className="text-[23px] leading-none transition-transform active:scale-95 disabled:opacity-50"
        ...
        {rating !== null && (
          <span className="ml-1.5 text-[15px] text-[var(--app-copy-soft)]">{rating}/5</span>
        )}
      </div>

      {/* Yes/No 버튼 */}
      <div className="mt-3 flex items-center gap-2">
        <span className="text-[15px] text-[var(--app-copy-soft)]">도움됐어요?</span>
        <button
          type="button"
        ...
        <div className="mt-3">
          {!showCommentInput ? (
            <button
              ...
              className="text-[15px] font-bold text-[var(--app-pink-strong)] underline underline-offset-2"
            >
              + 자세한 의견 남기기 (선택)
            </button>
          ) : (
            <div>
              <label
                ...
                className="text-[15px] font-bold text-[var(--app-copy-soft)]"
              >
                자세한 의견 (선택, 200자 이내)
              </label>
              <textarea
                ...
                className="mt-1 w-full resize-none rounded-[8px] border border-[var(--app-line)] bg-white p-2 text-[14.4px] text-[var(--app-ink)] placeholder:text-[var(--app-copy-soft)] focus:border-[var(--app-pink-strong)] focus:outline-none"
                disabled={state === 'saving'}
              />
              <div className="mt-1 flex items-center justify-between">
                <span className="text-[15px] text-[var(--app-copy-soft)]">
                  {comment.length}/200
                </span>
                <button
                  type="button"
      ...
      {/* 상태 메시지 */}
      {state === 'saved' && (
        <div className="mt-2.5 text-[15px] text-[var(--app-jade,#3F8796)]" role="status">
