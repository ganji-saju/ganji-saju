...
function StoryList({
  ...
  if (visibleItems.length === 0) {
    return (
      <div className="rounded-[1rem] border border-[var(--app-line)] bg-[rgba(255,255,255,0.035)] px-3 py-2 text-[15px] leading-5 text-[var(--app-copy-muted)]">
  ...
  return (
    <div className="grid gap-2">
      {visibleItems.map((item) => (
        <div
          key={item}
          className={cn('rounded-[1rem] border px-3 py-2 text-[15px] leading-5', colorClass)}
        >
          {item}
        </div>
      ))}
...
function StoryInlineFact({
  ...
  return (
    <div className={cn('rounded-[0.95rem] border px-3 py-2', colorClass)}>
      <div className="text-[15px] font-semibold opacity-80">{label}</div>
      <div className="mt-1 text-[15px] leading-5 text-[var(--app-copy)]">{value}</div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {score !== null ? (
          <span className="rounded-full border border-[var(--app-line)] bg-[rgba(255,255,255,0.035)] px-2 py-0.5 text-[15px] text-[var(--app-copy)]">
            {score}점
          </span>
        ) : null}
      </div>
      ...
      render: () => (
        <StoryBody className="content-start">
          <div className="rounded-[1.2rem] border border-[var(--app-gold)]/24 bg-[var(--app-gold)]/8 px-4 py-4">
            <p className="text-[15px] font-semibold tracking-[0.14em] text-[var(--app-gold-text)]">
              지금 핵심 한 줄
            </p>
          ...
          <details className="group rounded-[1rem] border border-[var(--app-line)] bg-[rgba(255,255,255,0.03)] px-3 py-3">
            <summary className="cursor-pointer list-none text-[15px] font-semibold text-[var(--app-gold-text)]">
              사주 기본표 함께 보기
            </summary>
            <div className="mt-3">
        ...
        <StoryBody className="content-start">
          ...
          {monthlyTimeline ? (
            <div className="rounded-[1rem] border border-[var(--app-gold)]/18 bg-[var(--app-gold)]/7 px-3 py-3">
              <div className="flex items-center gap-2 text-[15px] font-semibold tracking-[0.14em] text-[var(--app-gold-text)]">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <div className="mb-1 text-[15px] font-semibold text-[var(--app-jade)]">좋은 날짜</div>
              <StoryList items={luckyDates} tone="jade" max={2} />
            </div>
            <div>
              <div className="mb-1 text-[15px] font-semibold text-[var(--app-coral)]">주의 날짜</div>
        ...
        <StoryBody className="content-start">
          ...
          <div className="flex flex-wrap gap-2">
            {supportElements.slice(0, 3).map((item) => (
              <span
                key={item}
                className="rounded-full border border-[var(--app-line)] bg-[rgba(255,255,255,0.035)] px-3 py-1 text-[15px] text-[var(--app-copy)]"
              >
                {item}
              </span>
            ))}
        ...
        <StoryBody className="content-start">
          ...
          <div className="grid gap-2">
            {TRUST_SIGNALS.slice(0, 3).map((signal) => (
              ...
              >
                <div className="text-[15px] font-semibold text-[var(--app-ivory)]">{signal.title}</div>
                <p className="mt-1 text-[15px] leading-5 text-[var(--app-copy-muted)]">{signal.body}</p>
              </div>
            ))}
            <Link
        ...
        >
          <div className="saju-mobile-story-card-head">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[15px] leading-5 text-[var(--app-copy-muted)]">
                {activeSlide.summary}
              </p>
            </div>
            <div className="flex shrink-0 flex-col items-end gap-2">
              <span className="rounded-full border border-[var(--app-line)] bg-[rgba(255,255,255,0.04)] px-2.5 py-1 text-[15px] text-[var(--app-copy-muted)]">
