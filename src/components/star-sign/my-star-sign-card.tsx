...
export function MyStarSignCard({ profile, variant = 'default' }: Props) {
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        <span className="text-[25.3px]" style={{ color: elementHex }} aria-hidden="true">
          {meta.symbol}
        </span>
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex items-center gap-3">
        <span className="text-[36.8px]" style={{ color: elementHex }} aria-hidden="true">
          {meta.symbol}
        </span>
        <div className="min-w-0 flex-1">
        ...
        <div className="text-right">
          <div className="text-[11.5px] uppercase font-bold text-[var(--app-copy-soft)]">
            오늘
          </div>
      ...
      <p
        className="mt-2.5 text-[14.4px] leading-[1.55] text-[var(--app-ink)]"
        style={{ wordBreak: 'keep-all' }}
      >
        {fortune.highlight}
      </p>
      <div className="mt-3 flex flex-wrap gap-1.5">
        <span
          className="rounded-full border bg-white px-2 py-0.5 text-[12.1px] font-bold text-[var(--app-pink-strong)]"
          style={{ borderColor: 'var(--app-pink-line)' }}
        >
      ...
      <div
        className="mt-2 text-[12.1px] font-bold text-[var(--app-pink-strong)]"
      >
        상세 흐름·럭키·호환 보기 →
      </div>
    </Link>
