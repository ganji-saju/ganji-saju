  ...
  return (
    <div className={`overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm ${className}`}>
      <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
        <div>
          <p className="mb-0.5 text-xs text-gray-400">왜 이 점수가 나왔는지</p>
        ...
        <div className="text-right">
          ...
          <div className={`mt-0.5 text-xs font-medium ${colors.text}`}>{score.label.title}</div>
        </div>
      </div>
        ...
        {FACTOR_META.map((meta) => {
          ...
          return (
            <div key={meta.id} className="px-5 py-4">
              ...
              <p className="mb-2 pl-6 text-xs text-gray-500">{meta.subtitle}</p>
