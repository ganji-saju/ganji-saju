...
//   spec §8: 키 = (birth + gender + context_hash). 컨텍스트(관계/직업/고민) 변경 시에만 재생성.
import { createHash } from 'node:crypto';
import type { SajuDataV1 } from '@/domain/saju/engine/saju-data-v1';
import type { SajuDataV2 } from '@/domain/saju/engine/saju-data-v2-upgrade';

/** 프롬프트/스키마 버전 — 변경 시 캐시 무효화. */
export const TOTAL_REVIEW_PROMPT_VERSION = 'total-review/v1';
...
/**
 * 총평 캐시 키. 같은 사주 + 같은 성별 + 같은 컨텍스트 → 같은 결과 재사용.
...
export function buildTotalReviewCacheKey(
  ...
  const payload = JSON.stringify({
    ...
    context: {
      relationshipStatus: context.relationshipStatus ?? null,
