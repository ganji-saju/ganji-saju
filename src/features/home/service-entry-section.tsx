    ...
    <section id="core-service" className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
      <div className="mb-10 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-3">
          <p className="text-base uppercase tracking-[0.24em] text-[#d2b072]/75">Core Service</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_1.2fr_1fr]">
        {SERVICE_ENTRY_CARDS.map((card, index) => (
          ...
          >
            <div className="text-sm uppercase tracking-[0.2em] text-[#d2b072]/78">{card.eyebrow}</div>
            <h3 className="mt-4 text-3xl font-semibold text-[#f8f1df]">{card.title}</h3>
            <p className="mt-4 flex-1 text-base leading-7 text-white/58">{card.body}</p>
            <Link
              href={card.href}
              className={cn(
