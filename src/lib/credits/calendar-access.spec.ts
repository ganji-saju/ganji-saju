/**
 * Task 11: unlockFortuneCalendarMonth — 멤버십 게이트 단위 테스트
 *
...
import { unlockFortuneCalendarMonth } from './calendar-access';

// ── Supabase 클라이언트 팩토리 ───────────────────────────────────────────────
// hasFortuneCalendarMonthAccess: .from().select().eq().eq().eq().contains().limit(1)
// recordFortuneCalendarMonthAccess: .from().insert({...})
...
describe('unlockFortuneCalendarMonth — 멤버십 게이트', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  // ── (1) 기존 접근(reused) ─────────────────────────────────────────────────
  it('이미 접근한 경우: reused:true, 멤버십/전 미호출', async () => {
  ...
  it('premium 멤버: viaMembership:true, consumeMemberBenefit/전 미호출', async () => {
    ...
    // 무제한(calendarMonthly=null) 이므로 consumeMemberBenefit 호출 없음
    expect(consumeMemberBenefit).not.toHaveBeenCalled();
    // 전 경로 미호출
    expect(unlockCreditsOnce).not.toHaveBeenCalled();
    expect(deductCredits).not.toHaveBeenCalled();
