...
function createMockClient(
  table: Record<string, MockResponse | MockResponse[]>,
...
test('buildOperationsSnapshot - 결제는 payment_orders 완료 상태 기준(원화 합산)', async () => {
  ...
  const client = createMockClient({
    payment_orders: [
      ...
      // lifetime — RPC(payment_order_totals) 미정의 mock 이라 행 페이지네이션 폴백 경로.
      {
        data: [
...
test('buildOperationsSnapshot - 누적 결제: RPC(payment_order_totals) 우선', async () => {
  const now = new Date().toISOString();
  const client = createMockClient(
    {
      // window 결제 행만 존재 — lifetime 은 RPC 가 담당(행 fetch 없어야 함).
      payment_orders: {
        data: [{ user_id: 'u1', amount: 9900, created_at: now }],
      },
    },
    {
      payment_order_totals: {
        data: [{ order_count: 777, total_amount: 1234500 }] as unknown as Row[],
      },
    }
  );
  const snap = await buildOperationsSnapshot(client);
  assert.equal(snap.lifetime.totalPurchases, 777);
  assert.equal(snap.lifetime.totalPurchaseAmountWon, 1234500);
  // window 지표는 여전히 행 기반.
...
test('buildOperationsSnapshot - 방문자: RPC 결과 반영', async () => {
  ...
  const client = createMockClient(
    {},
    {
      site_visit_daily_counts: {
        data: [
          { date_key: kstToday, visitors: 123, page_views: 200 },
        ] as unknown as Row[],
      },
    }
  );
