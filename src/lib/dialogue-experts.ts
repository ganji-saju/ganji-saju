...
export const DIALOGUE_EXPERTS: readonly DialogueExpertMeta[] = [
  ...
  },
  {
    ...
    label: '별자리',
    topic: '별자리, 오늘의 흐름',
    description: '12별자리 기준으로 오늘의 분위기와 흐름을 가볍게 봅니다.',
...
const DIALOGUE_EXPERT_RAG_OVERLAYS: Record<DialogueExpertId, DialogueExpertRagOverlay> = {
  ...
  horse: {
    visibleOpening: '길일말선생 기준으로는 그 일에 좋은 날과 한 번 더 확인할 날을 나눠 봅니다.',
    ...
    actionPattern: ['막연한 길흉 대신 확인 가능한 날짜·조건 기준을 줍니다', '큰 결정은 후보 날짜를 좁히는 순서로 안내합니다'],
  ...
  rooster: {
    visibleOpening: '별닭선생 기준으로는 오늘 별자리 흐름에서 가볍게 챙길 한 가지를 봅니다.',
    ...
    avoid: ['별자리를 운명처럼 단정하지 않습니다', '불안을 주는 표현을 쓰지 않습니다'],
  },
  dog: {
    visibleOpening: '상담멍선생 기준으로는 무엇이든 편하게 꺼내면 지금 마음부터 같이 정리해요.',
