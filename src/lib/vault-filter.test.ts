...
test('vaultCategoryForProductId: tarot-daily → tarot(타로)', () => {
  assert.equal(vaultCategoryForProductId('tarot-daily'), 'tarot');
});
...
test('isVaultFilterKey: 타로 포함 유효 키 판별', () => {
  ...
  assert.equal(isVaultFilterKey('tarot'), true);
  assert.equal(isVaultFilterKey('xyz'), false);
  assert.equal(isVaultFilterKey(undefined), false);
});

test('normalizeVaultFilter: 유효 키(타로 포함)는 유지, 그 외(빈값/invalid)는 all 로 폴백', () => {
  ...
  assert.equal(normalizeVaultFilter('tarot'), 'tarot');
...
test('visibleVaultFilters: 전체 + 데이터 있는 카테고리만(정의 순서 유지)', () => {
  assert.deepEqual(
    visibleVaultFilters(['saju', 'today']).map((f) => f.key),
    ['all', 'saju', 'today']
  );
  assert.deepEqual(
    visibleVaultFilters(['tarot']).map((f) => f.key),
    ['all', 'tarot']
  );
  // 순서는 항상 전체 → 사주 → 오늘운세 → 궁합 → 타로 (정의 순서 유지)
  assert.deepEqual(
    visibleVaultFilters(['tarot', 'gunghap', 'today', 'saju']).map((f) => f.key),
    ['all', 'saju', 'today', 'gunghap', 'tarot']
  );
  assert.deepEqual(
