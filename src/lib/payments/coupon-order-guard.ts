...
/**
 * 사용자 안내. "이번 요청으로" 가 핵심 — 이 요청은 승인하지 않았지만, 환불된 주문이나 앞선 승인 타임아웃(실제로는 승인됨)처럼
 * 이전에 돈이 움직였을 수 있는 경우까지 "청구 없음"이라고 단정하면 틀린다(2026-09-12 재검증).
 */
export function approvalBlockMessage(block: ApprovalBlock): string {
  if (block === 'order_closed') {
    return '이미 끝난 주문(시간 초과·취소·환불)이라 결제하지 않았어요. 결제 화면을 다시 열어 주세요. 이번 요청으로 청구된 금액은 없습니다.';
  }
