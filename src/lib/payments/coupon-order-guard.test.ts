...
test('PG 승인 호출이 있는 모든 파일은 그보다 먼저 관문을 돌리고, 막히면 상태를 바꾸지 않는다', () => {
  ...
  for (const file of callers) {
    ...
    const expected = file.endsWith('reconciliation.ts')
      ? /status !== 'DONE' && !terminalFailureStatus\(input\.payment\.status\)\) \{\s*const approvalBlock = await checkBeforePgApproval\(input\.order, \{ approvalMayHaveBeenRequested: false \}\);\s*if \(approvalBlock\) return /
      : /const approvalBlock = await checkBeforePgApproval\(order, \{ approvalMayHaveBeenRequested: Boolean\(order\.paymentKey\) \}\);\s*if \(approvalBlock\) \{/;
