...
function buildHookSentence(
  ...
  return `${personalCue}${ageRange}의 ${ganzi} 대운은 ${concernCue}10년 흐름을 새로 잡는 챕터입니다.`;
}
  ...
  if (stemLabel && branchLabel) {
    head =
      stemLabel === branchLabel
        ? `${head_open(cycle)} ${stemLabel} 기운이 위아래로 겹쳐 강하게 들어오는 흐름입니다.`
        : `${head_open(cycle)} ${stemLabel} 기운과 ${branchLabel} 기운이 함께 들어오는 흐름입니다.`;
  } else if (stemLabel || branchLabel) {
    head = `${head_open(cycle)} ${stemLabel || branchLabel} 기운이 들어오는 흐름입니다.`;
  } else {
...
const RELATIONSHIP_BASELINE_BY_SIPSIN: Record<TenGodCode, string> = {
  ...
  정관: '이 10년은 자리·역할·인정의 기운이 강해져, 가까운 사람과의 관계에서도 역할 기대가 또렷해집니다. 책임감을 나눠 적어두면 무거움이 분산돼요.',
  편인: '이 10년은 혼자만의 시간·공부가 늘어나는 흐름이라, 가까운 사람이 본인의 깊이를 이해 못 한다고 느낄 수 있어요. 의도적으로 짧은 공유를 자주 하면 거리감이 좁혀집니다.',
  정인: '이 10년은 돌봄·후원·배움의 인연이 늘어나는 흐름이에요. 누군가에게 받기도 하고, 누군가를 챙기게도 됩니다. 멘토·선배 키워드가 관계를 관통합니다.',
};
...
function buildRelationshipText(
  ...
  if (status === 'dating') {
    ...
    statusBody = '솔로 상태라면 인연이 들어오는 흐름이 평소와 달라집니다. 안 만나본 유형의 사람과의 첫 만남을 너무 빠르게 닫지 마세요.';
...
const WEALTH_BASELINE_BY_SIPSIN: Record<TenGodCode, string> = {
  ...
  정관: '이 10년은 자리·직책·인정의 기운이 강해져, 안정된 직장이나 책임 있는 역할이 본인 색에 맞게 들어옵니다. 평가를 받을 수 있는 기회를 적극 잡으세요.',
  편인: '이 10년은 남다른 시각·전문성이 돈으로 연결되는 흐름이에요. 메인 직무 외 부업·강의·콘텐츠 같은 사이드 수입원이 자연스럽게 생깁니다.',
  정인: '이 10년은 누군가에게 배우고 후원을 받는 기운이 강해, 학위·자격증·강의 같은 인풋이 수익으로 이어지는 구조가 형성됩니다.',
};

const WEALTH_BASELINE_DEFAULT =
  '이 10년은 돈과 일의 흐름을 함께 재편합니다.';
...
// 2026-05-25: naming-policy §2 — 자연 비유(새싹/햇살/흙/쇠/물) 폐기, 표준 "목/화/토/금/수 기운"으로 통일.
export const SHORTAGE_ACTION_DICT: Record<Element, PracticalAction> = {
  목: {
    reason: '목 기운이 약해 새로 시작하고 성장하는 힘이 흐려지는 패턴',
    what: '새 일·공부 한 가지를 작게 시작해 성장 기운을 채우기',
    how: '매주 새 책 1장씩 + 안 만나본 유형의 사람 1명. 시작을 미루지 않는 것이 핵심.',
  },
  화: {
  ...
  토: {
    reason: '토 기운이 약해 생활 기반과 리듬이 흔들리는 패턴',
  ...
  수: {
    reason: '수 기운이 약해 정보·휴식·생각 정리가 부족한 패턴',
...
const TEN_GOD_ACTION_DICT: Record<TenGodCode, PracticalAction> = {
  비견: {
    reason: `${glossaryHint('비견')} 기운이 강해 동등한 유형의 사람과 부딪치고 협업/경쟁이 동시에 늘어남`,
    what: '역할 경계를 분명히 그어두기',
    how: '공동 프로젝트는 계약서·역할표 1장 먼저. "친하니까" 로 시작하지 말 것.',
  },
  겁재: {
    reason: `${glossaryHint('겁재')} 기운이 강해 가까운 사람과 돈·기회를 나누다 갈등이 생기기 쉬움`,
    what: '금전 거리를 미리 두기',
  ...
  편재: {
    reason: `${glossaryHint('편재')} 기운이 강해 기회와 사람을 넓게 보다가 흩어지기 쉬움`,
    what: '기회 1개로 좁히기',
    how: '확장 전 3개월 손익 점검. 매출보다 단가·고정비 우선 정리.',
  },
  정재: {
    reason: `${glossaryHint('정재')} 기운이 강해 안정적으로 축적하지만 한 번 잘못 분산되면 회복이 느림`,
    what: '구조 점검 + 분산 정리',
  ...
  편인: {
    reason: `${glossaryHint('편인')} 기운이 강해 직관과 깊이는 살아나지만 외부 연결이 끊기기 쉬움`,
    what: '주 1회 자료 정리 + 1명 외부 연결',
    how: '주간 메모를 한 페이지에 정리하고 외부 1명과 짧게 공유.',
  },
  정인: {
    reason: `${glossaryHint('정인')} 기운이 살아 있어 후원·배움이 자기 보강으로 이어짐`,
    what: '도움 받기 + 베풀기 동시',
...
const CYCLE_SIPSIN_ACTION_DICT: Record<TenGodCode, PracticalAction> = {
  비견: {
    reason: '이 10년에 비슷한 성향의 동료·라이벌이 많아지는 패턴',
 ...
 *
 * 2026-05-20 export: 회귀 가드 테스트 공유.
 */
export const CHAPTER_PATTERN_TEMPLATES: Record<ChapterPattern, string[]> = {
  signal: [
    '새로운 10년의 문이 열리는 분기점',
  ...
  fomoAd: [
    ...
    '추진력이 모이는 시기,출발선에 서 보기 좋은 시기',
    '드러내기 좋은 시기,평소 준비를 꺼내볼 시기',
  ],
  hope: [
    '추진력이 다시 살아나는 시기,차근차근 출발하기 좋은 시기',
    '받쳐주는 흐름이 들어오는 시기,작은 한 걸음으로 시작',
    '회복과 정리가 끝나고 본격 도약이 시작되는 시기',
  ],
  transform: [
    '현실적인 결과물이 자리잡는 시기,수확 단계로 가는 10년',
  ...
  const transitionPart =
    transitionPhase === 'entering'
      ...
      : transitionPhase === 'leaving'
        ? ' 지금은 운이 마무리되는 길목 — 이번 대운이 마무리되는 ±1년이라 몸·마음·일이 새로 정돈되는 시기입니다. 정리 우선, 새 시작은 다음 대운에서.'
        : '';
