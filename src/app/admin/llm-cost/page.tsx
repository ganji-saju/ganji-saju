...
export default async function LlmCostPage() {
  ...
  return (
    <AdminPage title="LLM 비용">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ...
              <div key={m.label} className="rounded-[12px] border border-[var(--app-line)] p-3">
                <div className="text-[11.5px] text-[var(--app-copy-soft)]">{m.label}</div>
        ...
        <Card title="일별 비용 추이">
          {daily.length === 0 ? (
            <p className="text-[13px] text-[var(--app-copy-soft)]">데이터 없음.</p>
          ) : (
            <ul className="space-y-1">
              {daily.map((d) => (
                <li key={d.date} className="flex items-center gap-2">
                  <span className="w-[78px] shrink-0 text-[13px] text-[var(--app-copy-soft)]">
                    {d.date.slice(5)}
                  </span>
