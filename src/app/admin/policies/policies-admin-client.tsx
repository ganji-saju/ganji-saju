    ...
    <div className="grid gap-4 md:grid-cols-[220px_1fr]">
        ...
        {kinds.map((k) => {
          ...
          return (
            <button
              ...
              className={
                'block w-full rounded-[10px] border px-3 py-2 text-left text-[15px] leading-[1.5] ' +
                (isSelected
      ...
      <section className="space-y-4">
        <div className="rounded-[14px] border bg-white p-4" style={{ borderColor: 'var(--app-line)' }}>
          <h2 className="text-[16.1px] font-extrabold">
            {POLICY_LABELS[selectedKind]}{' '}
            <a
              href={POLICY_URLS[selectedKind]}
              target="_blank"
              rel="noopener noreferrer"
              className="ml-2 text-[12.6px] font-normal text-[var(--app-copy-muted)] underline"
            >
              사용자 페이지 열기 →
            </a>
          </h2>
        ...
        >
          <h3 className="text-[15px] font-extrabold">신규 버전 생성</h3>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                placeholder="v1.0.0"
                className="mt-1 w-full rounded border px-3 py-2 text-[16.1px]"
                style={{ borderColor: 'var(--app-line)' }}
              />
            </label>
            <label className="block">
          ...
          <label className="block">
            ...
            <textarea
              ...
              className="mt-1 w-full rounded border px-3 py-2 text-[15px] font-mono leading-[1.6]"
              style={{ borderColor: 'var(--app-line)' }}
            />
          </label>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="mt-7 flex items-center gap-2 text-[15px]">
              <input
                type="checkbox"
          ...
          <div className="flex items-center gap-3">
            <button
              type="submit"
              disabled={submitting}
              className="rounded-[10px] bg-[var(--app-pink-strong)] px-4 py-2 text-[15px] font-bold text-white disabled:opacity-50"
            >
            ...
            {message && (
              <span
                className={
                  message.type === 'ok'
                    ? 'text-[13.8px] text-[var(--app-jade)]'
                    : 'text-[13.8px] text-[var(--app-pink-strong)]'
                }
              >
                {message.text}
              </span>
        ...
        {/* 이력 */}
        <div className="rounded-[14px] border bg-white p-4" style={{ borderColor: 'var(--app-line)' }}>
          <h3 className="text-[15px] font-extrabold">버전 이력</h3>
          {history.length === 0 ? (
            <p className="mt-2 text-[13.8px] text-[var(--app-copy-muted)]">이력 없음.</p>
          ) : (
            <table className="mt-2 w-full text-[13.8px]">
              <thead>
