...
export function SajuVerifyComparePanel({ expected }: { expected: ExpectedValues }) {
  ...
  return (
    <section className="rounded-[18px] border bg-white p-5" style={{ borderColor: 'var(--app-pink-line)' }}>
      ...
      <p className="mt-1.5 text-[13px] leading-[1.65] text-[var(--app-copy-muted)]">
      ...
      <textarea
        ...
        className="mt-3 min-h-[200px] w-full rounded-[12px] border bg-white p-3 text-[14px] leading-[1.7] text-[var(--app-ink)] outline-none placeholder:text-[var(--app-copy-soft)]"
      ...
      {results.length > 0 ? (
        <>
          ...
          >
            ...
            <h3 className="mt-1 text-[16px] font-extrabold leading-[1.4] text-[var(--app-ink)]">
              필수 {requiredPassed}/{required.length} 매칭{' '}
              {optionalTotal > 0 ? `· 선택 ${optionalPassed}/${optionalTotal}` : ''}
              {requiredPassed === required.length ? ' ✓' : ' ⚠'}
            </h3>
            <p className="mt-1 text-[14px] leading-[1.65] text-[var(--app-copy)]">
              {requiredPassed === required.length
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {result.optional ? (
                        <span
                          className="rounded-full px-1.5 py-0.5 text-[11px] font-extrabold text-[var(--app-copy-muted)]"
                    ...
                    <div className="mt-0.5 text-[13px] text-[var(--app-copy-muted)]">
                      예상값: <strong className="text-[var(--app-ink)]">{result.expected}</strong>
                    </div>
                  </div>
                  <span
                    className="shrink-0 inline-flex h-7 items-center rounded-full border px-2.5 text-[13px] font-extrabold"
                    style={
                      result.found
                        ? {
                            background: '#e8f5ee',
                ...
                {result.matchedTokens.length > 0 ? (
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {result.matchedTokens.map((token) => (
                      <span
                        key={token}
                        className="rounded-full bg-[var(--app-pink-soft)] px-2 py-0.5 text-[11.5px] font-extrabold text-[var(--app-pink-strong)]"
                      >
                        {token}
                      </span>
                    ))}
