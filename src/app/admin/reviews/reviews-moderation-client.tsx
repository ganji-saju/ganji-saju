...
export function ReviewsModerationClient({
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        {FILTERS.map((f) => (
          <button
            ...
            className="rounded-full border px-3 py-1.5 text-[14px] font-extrabold transition"
            style={{
        ...
        <span className="ml-2 text-[13px] font-bold text-[var(--app-copy-muted)]">
          총 {total}건
        </span>
      </div>
      ...
      {loading ? (
        <div className="rounded-[14px] border border-dashed border-[var(--app-line)] bg-white p-6 text-center text-[14px] text-[var(--app-copy-muted)]">
          불러오는 중...
        </div>
      ) : reviews.length === 0 ? (
        <div className="rounded-[14px] border border-dashed border-[var(--app-line)] bg-white p-6 text-center text-[14px] text-[var(--app-copy-muted)]">
        ...
        <div className="grid gap-3">
          {reviews.map((review) => (
            ...
            >
              <header className="flex flex-wrap items-center justify-between gap-2 text-[13px]">
                <div className="flex flex-wrap items-center gap-2">
                  <span
              ...
              <p
                className="mt-3 text-[14px] leading-[1.65] text-[var(--app-copy)]"
                style={{ wordBreak: 'keep-all' }}
              >
                {review.content}
              </p>
              <footer className="mt-3 flex items-center justify-between gap-2 text-[13px]">
                <span className="font-bold text-[var(--app-copy-muted)]">
              ...
              {review.moderationNote ? (
                <div className="mt-2 rounded-[8px] bg-[var(--app-pink-soft)]/50 px-2.5 py-1.5 text-[13px] font-bold text-[var(--app-copy-muted)]">
