    ...
    <AdminPage title="세그먼트·코호트">
        ...
        <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {counts.map(({ segment, count }) => (
            ...
            >
              <div className="text-[13px] text-[var(--app-copy-soft)]">{segment.label}</div>
              <div className="text-[20px] font-extrabold text-[var(--app-ink)]">{count.toLocaleString('ko-KR')}</div>
              <div className="mt-1 text-[13px] text-[var(--app-copy-soft)]">{segment.description}</div>
            </Link>
          ))}
        </section>

        <section className="rounded-[14px] border border-[var(--app-line)] bg-white p-4">
          <h2 className="text-[14px] font-extrabold text-[var(--app-ink)]">가입 코호트 잔존율</h2>
          <p className="mt-1 text-[13px] text-[var(--app-copy-soft)]">
            DN = 가입 후 N일 이상 활동 유지 비율(프록시). 성숙 코호트만 표시, 그 외 —. 표본 적을 땐 참고치.
          </p>
          {cohorts.length === 0 ? (
            <p className="mt-3 text-[13px] text-[var(--app-copy-soft)]">데이터가 없습니다.</p>
          ) : (
