  ...
  return (
    <AdminPage title="가입자 관리">
        ...
        <form method="get" className="rounded-[14px] border border-[var(--app-line)] bg-white p-4">
          <label htmlFor="q" className="text-[14px] font-extrabold text-[var(--app-ink)]">
            이메일 또는 UUID 빠른검색
          </label>
          <div className="mt-2 flex gap-2">
            <input
              ...
              className="flex-1 rounded-[10px] border border-[var(--app-line)] px-3 py-2 text-[14px]"
            />
            <button type="submit" className="rounded-[10px] bg-[var(--app-pink-strong)] px-4 py-2 text-[14px] font-extrabold text-white">
              검색
            </button>
          </div>
          {query && (
            <ul className="mt-3 space-y-2">
              {searchResults.length === 0 && (
                <li className="text-[13px] text-[var(--app-copy-soft)]">일치하는 사용자가 없습니다.</li>
              )}
              {searchResults.map((u) => (
                <li key={u.id}>
                  <Link href={`/admin/users/${u.id}`} className="block rounded-[12px] border border-[var(--app-line)] p-3 hover:bg-[var(--app-pink-soft)]">
                    <span className="text-[14px] font-bold">{maskEmail(u.email, role) ?? u.id}</span>
                    <span className="ml-2 text-[13px] text-[var(--app-copy-soft)]">{fmtDate(u.createdAt)}</span>
                  </Link>
                </li>
              ))}
            </ul>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {SEGMENTS.map((s) => (
            <Link
              ...
              title={s.description}
              className="rounded-full border border-[var(--app-line)] px-3 py-1 text-[13px] text-[var(--app-ink)] hover:bg-[var(--app-pink-soft)]"
            >
              {s.label}
            </Link>
          ))}
          <Link
            ...
            className="rounded-full border border-[var(--app-pink-strong)] px-3 py-1 text-[13px] font-extrabold text-[var(--app-pink-strong)]"
          >
            세그먼트·코호트 개요 →
          </Link>
        </div>

        <form method="get" className="rounded-[14px] border border-[var(--app-line)] bg-white p-4 grid grid-cols-2 gap-3 md:grid-cols-4 text-[13px]">
        ...
        <section className="rounded-[14px] border border-[var(--app-line)] bg-white p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-[14px] font-extrabold">
              ...
              {items.length < totalCount ? (
                <span className="ml-1 text-[13px] font-bold text-[var(--app-copy-soft)]">
                  (표시 {items.length})
                </span>
              ) : null}
            </h2>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span
                className={`text-[13px] ${summaryStale ? 'font-extrabold text-[var(--app-coral)]' : 'text-[var(--app-copy-soft)]'}`}
              >
              ...
              <a href={`/api/admin/users/export?${usp.toString()}`} className="rounded-[10px] border px-3 py-1 text-[13px]">CSV 내보내기</a>
            </div>
          </div>

          {items.length === 0 ? (
            <div className="mt-4 rounded-[12px] border border-dashed border-[var(--app-line)] p-6 text-center">
              <p className="text-[14px] font-bold">조건에 맞는 가입자가 없어요</p>
              <p className="mt-1 text-[13px] text-[var(--app-copy-soft)]">필터를 바꾸거나 세그먼트를 눌러보세요.</p>
              <Link href="/admin/users" className="mt-3 inline-block rounded-[10px] border px-3 py-1 text-[13px]">필터 초기화</Link>
            </div>
          ) : (
                  ...
                  {items.map((it) => (
                    <tr key={it.userId} className="border-t border-[var(--app-line)]">
                      <td className="py-2">
                        ...
                        <div className="text-[13px] text-[var(--app-copy-soft)]">{it.email ?? '—'}</div>
                      </td>
          ...
          {nextHref && (
            <div className="mt-4 text-center">
              <Link href={nextHref} className="rounded-[10px] border px-4 py-2 text-[13px]">다음 페이지 ›</Link>
            </div>
          )}
        </section>
    </AdminPage>
