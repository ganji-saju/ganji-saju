...
vi.mock('@/lib/admin/external-report-history', () => ({ saveExternalReport: vi.fn() }));
...
const saved = { id: 'aa809a61-df81-4cac-9c96-e4cd1a5db76c', createdAt: '2026-09-17T15:30:00.000Z' };
...
describe('POST /api/admin/external-report', () => {
  beforeEach(() => {
    ...
    vi.mocked(saveExternalReport).mockResolvedValue(saved);
  });
  ...
  it.each([
    ...
    expect(saveExternalReport).not.toHaveBeenCalled();
  ...
  it('does not persist a finished generation when cancellation arrives before saving', async () => {
    const controller = new AbortController();
    vi.mocked(generateExternalReport).mockImplementationOnce(async () => {
      controller.abort();
      return generated;
    });
  ...
  it('returns a private no-store failure when saving fails and releases the generation guard', async () => {
    vi.mocked(saveExternalReport).mockRejectedValueOnce(new Error('구매자 1982-01-29 snapshot database detail'));
    const response = await POST(request());
    expect(response.status).toBe(500);
    expect(response.headers.get('cache-control')).toBe('no-store');
    const body = await response.json();
    expect(body).toEqual({ ok: false, error: expect.stringContaining('기록 저장') });
    expect(JSON.stringify(body)).not.toMatch(/구매자|1982|snapshot|database/);
    expect(logAdminAccess).not.toHaveBeenCalled();
    expect((await POST(request())).status).toBe(200);
    expect(saveExternalReport).toHaveBeenCalledTimes(2);
  });
});
