...
> ⚠️ **아래 SQL 은 설계 초안이다 — 정본은 마이그레이션 원문(079·080·085).** 실제와 다른 곳: `identity_hash`·사람당 유니크 없음(결정 B),
> code 는 `[0-9]{4}` 만, 유니크는 080 의 `one_live_per_user`(released 제외), `expires_at` 기본값 2027-12-31 KST 말일,
> 085 가 접두=등급 CHECK 와 anon·authenticated 테이블 권한 회수를 더했다.
...
> 보강 후 적대적 검증이 찾은 생존 회귀 12개까지 더해 **44/44 red · 정상 변경 6개 오탐 0**. 소스 스캔은 `src/lib/payments/coupon-chokepoint.test.ts`(이하 choke).
...
| 4 | **`prepare` 응답 `amount` === 생성된 `order.amount`** (타입이 못 잡는 자리) | choke — **성공 응답 객체 안**의 금액이 `order.amount` 하나뿐 + 멈춤 분기 3곳(쿠폰 미적용·금액 대조·귀속 실패)이 `blockPrepare` 로 끝난다 + 주문 금액·할인 스냅샷을 고치는 UPDATE 없음 |
| 5 | **`prepare` 가 body 에서 금액·할인율을 읽지 않는다** | choke — 읽는 키 **허용 목록** + 구조분해·전개·전달 금지 + 쿼리스트링 등 다른 입력 금지 |
| 6 | **`buildPriceDisplayMap` 입력에 user/coupon 인자가 없다** (전역 캐시 오염 방지) | choke — 체인 4파일(리졸버·맵·요청 캐시·Provider) 사용자·쿠폰·세션 금지 + 인자 0 시그니처 + 루트 레이아웃 정적(쿠키·인증·`dynamic`·`revalidate=0` 금지, 맵 무가공) + `<PriceProvider>` 는 루트 레이아웃 1곳뿐. (한계: 레이아웃이 렌더하는 **서버 컴포넌트**가 쿠키를 읽는 전이 경로는 못 본다 — 지금 자식은 전부 `'use client'`) |
...
| 10 | 만료된 쿠폰의 옛 `prepared` 주문이 승인 거부되는지(§5-3) | `coupon-order-guard.test.ts` (관문 결과 가공 금지, 줄바꿈 허용) + `order-ledger.snapshot.test.ts`(읽은 주문이 `couponCode` 를 안다) + choke(주문 원장 조회는 `select('*')` — 컬럼을 좁혀 `coupon_code` 가 빠지면 관문이 검사를 건너뛴다) |

같이 고정한 것(§3-3·§5-2·§7·§10): 결제 버튼 금액·요약 금액·`checkout_viewed` 값 = `quote.chargeAmount`, 서버 금액 검증 블록, 회수 판단용 주문 조회 실패는 "주문 있음"(요구 4),
전 차감·언락 경로의 할인쿠폰 모듈 import 금지, "전으로 여는 경우엔 적용되지 않아요" 안내, 결제내역 금액은 주문 원장 우선.
PR7 이 고친 버그(모두 번들 종합 리포트 — 판매 중·쿠폰 대상): 관리자 환불 목록에서 구성품이 지급된 번들 주문이 빠짐(환불 불가) · 사용자 목록 요약(`summary-refresh`)이 번들·고아 주문을 환불 가능액에서 뺌(인자 필수화 — 빠뜨리면 타입 오류) · 할인된 번들이 결제내역·LTV 에 정가로 표시.

## 14. 포기한 것
캠페인 관리 · 다중 쿠폰 스택 · 조건부 할인 · 쿠폰 이전.
> (정정 2026-09-13) v1 목록의 "배치 일괄 무효화"는 §11 B 위험의 완화책(배치 단위 `disabled_at` 회수)과 모순이라 **PR6 에서 구현했다**.

## 15. PR6 관리자 화면 `/admin/coupons` — 결정과 불변식 (2026-09-13)

**사용자 결정**(재논의 금지): ① [귀속 해제] = **계정을 푼다** — 보유자가 차지한 쿠폰에 `released_at`(종료). 코드는 영구 종료, 계정은
새 코드 사용 가능. 남에게 선점된 코드의 피해자는 **새 코드 1장 발급**으로 구제(`bound_user_id=null` 은 080 이 금지).
② 코드 목록 = **언제든 재다운로드**(super_admin, 다운로드마다 감사). ③ 범위 = 요율·상한·등급 on/off · 대량 발급 · 배치 현황/귀속 속도 ·
배치 회수 · **요율 소급 · 배치 만료 일괄 변경 · 회수 되살리기**. ④ 085(접두=등급 CHECK)는 사용자가 SQL Editor 로 머지 전 적용.

| 불변식 | 강제 |
|---|---|
| 쓰기는 **서버 액션**(Next 가 Origin=Host 검사), 모든 export 첫 줄 super_admin 재확인, 인자는 FormData | 소스 가드(가드 빠진 픽스처로 red 증명) |
| 코드 평문은 **CSV 내보내기 한 경로**로만, **감사 기록이 먼저 성공**해야 반환. 화면·발급 응답·감사 meta·오류 문구엔 없음(마스킹 `ganji-10-**34`) | `recordCouponAudit`(오류를 던짐) + 테스트 |
| 환경: 운영=전부 · staging·로컬(test)=`staging-test` 행만(쿼리 필터까지 이중) · preview/모름=전부 거부. 로컬이 Host 를 운영으로 바꿔도 `VERCEL_ENV≠production` 이면 거부. 요율·등급은 운영만 | `adminCouponEnv`·`batchAllowedInEnv` + 함수별 테스트 |
| 발급 = node:crypto 무작위(기존 번호 전량 keyset 제외) · 한 문장 insert · upsert 금지 · 배치명 NFC 정규화·중복 거부("기존 배치에 추가"만 허용) · 1회 1,000장 · 만료 내일~730일 | 테스트 + 085 |
| 목록 조회는 PK keyset(`order(code)`/`order(id)`) — 정렬 없는 페이지는 인쇄 CSV 에 코드가 두 번 실린다 | 가짜 DB 가 정렬 없는 limit 를 throw |
| 요율 변경은 `updated_at` 원문 CAS. 소급 = `released_at is null` 인 귀속 전부(회수·만료 포함). **소급 인하 뒤 옛 할인 주문은 승인 관문이 `coupon_rate_changed` 로 거부**(§5-3 보강) | 테스트(관문·소급) |
| 해제 = `(보유자, bound_at)` CAS — 코드를 몰라도 되고, 조회 뒤 다른 쿠폰으로 옮겼으면 0행 | 테스트 |
| 회수 = 그 배치의 살아 있는 행에만 이번 스탬프, 되살리기 = **그 스탬프 원문**만(µs). released 는 되살아나지 않는다 | 테스트 |
| 탈퇴 전 `releaseCouponsOfUser` — 주문 cascade 로 결제 증거가 지워져 24h 회수로 코드가 남에게 가던 구멍 | 삭제 경로 전수 가드 |
| 운영 배포에서 가치가 늘어나는 조작(발급·되살리기·만료·요율·내보내기)은 운영 메일 알림(세션 탈취 탐지) | 코드 |

**남은 위험**(기록): 관리자 화면에도 GTM·gtag 가 로드되고 CSP 는 Report-Only·세션 쿠키는 JS 로 읽힌다 — super_admin 세션 =
미사용 코드 전량(재다운로드 결정). `/admin` 경로의 GTM 제외는 별도 작업. 이름 확인→insert 사이의 두 관리자 동시 발급 경합은 막지 않는다.
