...
## 확정 결정 (2026-05-24)
- **D1 궁합 LLM:** ON — 프로덕션 Vercel env `OPENAI_INTERPRET_COMPATIBILITY=1` 설정 + 재배포 완료(ganjisaju.kr 적용). `.env.example` 문서화.
- **D2 띠운세 콘텐츠 소스:** **정적 수기 작성**(LLM 생성 아님).
- **D3 연생 범위:** 띠당 **5개 연생**(예: 닭띠 57/69/81/93/05).
- **D4 꿈해몽:** **정적 확충·연결 위주**(이번 범위에서 LLM 개인화 해석 미포함).
- **D5 꿈해몽 사전 규모:** **200+ 키워드**.
