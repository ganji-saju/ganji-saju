...
- **API 취소 통보의 orderId** 가 원주문인지 우리가 보낸 `cxl…_원주문` 인지. 운영 환불 5건은 전부 콘솔 취소가 먼저였다(통보가 환불 요청보다 먼저 옴 → "이미 취소됨" 우회로 완료). 그래서 API 취소가 성공한 적이 없다.
  → **다음 운영 관리자 환불 때** 그 결제키의 `payment_webhook_events` 한 줄을 보면 풀린다(새 결제 불필요). 가드는 orderId 대신 **tid(=paymentKey)로 주문을 찾게** 바꾸면 이 값과 무관해진다.
