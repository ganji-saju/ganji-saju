...
작성일: 2026-05-23 / 상태: **계획 전용 (코드 미수정)** / 출처: [`pricing-proposal.md`](pricing-proposal.md) 롤아웃 1~3순위 + 검증 지표
대상 커밋 기준: `1f65f40` (결제 P0 정비 직후)
...
1. **현재 결제 모델은 `1 패키지 = 1 productId = 1 scope`** 로 고정돼 있다(catalog·confirmation·confirm·scope·조회 전부 이 가정). 제안서의 묶음 상품(오늘 풀세트·올해 풀패키지)은 **`1 결제 = N 권한`** 이라, 이 모델에 없는 **묶음 인프라를 새로 도입**해야 한다.
2. 그래서 제안서의 **"1순위 = 난이도 낮음"은 카탈로그/카피 관점일 뿐**, 실제로는 결제 백엔드(`confirm` grant 경로) 변경이 처음으로 필요해 **실질 난이도는 "중"**이다.
3. **진짜 난이도 최저는 티어 C "분할 환산 문구"** (카피 1곳, 로직 0). 이건 즉시 가능.
4. 따라서 권장 실행 순서를 아래처럼 재배열한다:
   - **0순위**: 분할 환산 문구 (카피만, 코드 로직 0)
   - **1순위**: **묶음 인프라** + 오늘 풀세트 990원 (백엔드 최초 변경)
...
### 2.1 [난이도 ★★★☆☆ 중] 묶음 인프라 + 오늘 풀세트 990원 — 티어 A (1순위)

> 이 단계가 **묶음 인프라의 최초 도입**이다. 이후 2순위는 이걸 재사용한다.

- **무엇**: today-detail(550) + score-factor(550)를 990원에 묶음. 단, score-factor는 factor(F1~F5) 단위라 "어느 항목을 줄지" 정의 필요 → 오늘 풀세트는 **today-detail + score-factor F1** 또는 today-detail 단독 확장으로 범위 축소 권장(스펙에서 확정).
- **신규 설계 (A안)**:
  ...
  6. `paid-reading-snapshots.ts`: 묶음 대표 snapshot 1개 또는 구성품별. `getPaidProductTitle`에 묶음 항목 추가.
  7. **테스트(TDD 필수)**: 묶음 grant 멱등성, 부분보유 재결제, 구성품 scope 정확성.
- **영향 파일 수**: 약 6~7개 + 테스트.
- **리스크**: 결제 백엔드 첫 변경. 멱등/부분보유/환불 회수 엣지가 많음 → `superpowers:test-driven-development` 권장.
...
- 🟡 **score-factor 묶음 구성 모호**: 오늘 풀세트의 score-factor는 factor 단위라 "어느 항목" 정의 필요. 스펙에서 범위 축소 권장.
