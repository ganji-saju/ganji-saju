...
- **#509·#510·#511 나이스페이 실결제(live) 가동 — U116 디버깅**: 결제창·서명검증(sha256(authToken+clientId+amount+secretKey))은 통과하는데 마지막 승인 API(`POST /v1/payments/{tid}`)만 **U116 "사용자 정보가 존재하지 않습니다"**. 진단패치(#509 resultMsg 노출 → #510 clientKey vs clientId 비교 노출)로 실측. **근인 = 운영(live) 키인데 `NEXT_PUBLIC_NICEPAY_MODE=sandbox`** → 승인을 `sandbox-api.nicepay.co.kr` 로 보내 **운영모드로 생성된 TID 가 샌드박스에 없음** → U116. (나이스페이 거래상세 `Mode: 운영모드`가 결정 단서.) **MODE=live 로 맞춰 실결제 E2E 정상**(소액 실결제 → 결제완료 → 사주풀이). ⚠️ `NICEPAY_API_BASE` 가 설정되면 MODE 보다 우선(호스트 강제) → MODE 전환 무력화 함정. 결제창 host `web.nicepay.co.kr/v3/smart` 는 창 UI host 일 뿐 V2 연동 정상(V2/V3 문제 아님). #511 에서 진단노출 제거(사용자=일반문구, 서버로그·주문 error 엔 사유 보존). → [[reference_payment-env-toggle]]
