...
### 방식(조사 워크플로 2렌즈)
- `refundedMembershipRenewal`(순수): renews_at − 30일(결제 1건 = 항상 30일, 끝에 누적). 결과 ≤ 지금이면 `expired` + renews_at=지금(남기면 재구매 때 되살아남). renews_at 없음(무기한)은 손대지 않음.
  상태만 cancelled 는 혜택이 안 끊긴다(isEntitledStatus 가 cancelled 를 권한으로) — 그래서 안 씀.
- 훅은 `markPaymentOrderRefunded` 의 **방금 refunded 로 바뀐 분기**(GA 환불과 같은 자리) — 관리자·웹훅·정산이 겹쳐도 조건부 UPDATE 로 정확히 1회. 실패는 던진다(재호출이 안 오므로 드러내야).
- 나이스 취소의 전 회수를 지급 조건(`shouldGrantCredits`)과 같게.

### 검증
유닛 1,664 + node:test 191, tsc 0 · 뮤테이션 5/5 red(만료 분기·renews_at 옛 값·훅 위치·카탈로그 전 회수·차감 0일).

### 남은 것(보고)
관리자 "멤버십 해제"가 cancelled 라 혜택이 안 끊김(같은 원인 — 범위 확인 필요) · 지급 재시도로 activate 가 두 번 돈 결제(+60일)는 30일 남음 ·
멤버 경로로 이미 연 달력·상세 기록은 환불 뒤에도 열람(정책 판단) · 대화상담 질문 3회(taste_dialogue_entry)는 전 3개를 주는데 취소 시 회수 안 함(pkg.credits=0).
