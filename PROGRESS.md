...
## 2026-09-09 — 결제 화면 군더더기 축소: 동의 문안·신뢰안내 축소, 계좌이체 문구 제거
...
| 블록 | 전 | 후 |
|---|---|---|
| 결제 전 확인(동의) | 865px | **656px** (−24%) |
| 풀이 신뢰 안내 | 293px | **228px** (−22%) |

글자: 신뢰안내·주문요약 `14.4/13.2px → 10.5px`, 동의 문장 `14.4 → 11px`,
약관 링크 `12.6 → 10px`, 제목 `15 → 12px`, 체크박스 `18 → 15px`.
...
⚠️ **줄인 건 문장뿐이다.** 동의 대상 4종 링크는 바로 아래 그대로 남는다 — 이 컴포넌트 상단
주석의 제약("동의 대상을 화면에서 확인할 수 없으면 통합 동의의 유효성이 약해진다")을 지켰다.
...
| 위치 | 전 → 후 |
|---|---|
| checkout 흐름 안내 | '카드 또는 계좌이체로 결제합니다' → '카드·간편결제로 결제합니다' |
| 결제완료 화면 결제수단 | '카드 · 계좌이체' → '카드 · 간편결제' |
| 결제 버튼 하단 | '…카드와 계좌이체를 모두 지원합니다.' 문장 삭제 |
| `moonlight.CHECKOUT_METHODS` | 미사용 상수지만 되살아나지 않게 같이 정리 |

남은 '계좌이체'는 **주석과 토스 전용 옵션 라벨**뿐이다(토스 provider 에서는 여전히 정상 수단).

### 6px 는 넣지 않았다

요청은 "6px 정도"였지만 같은 문장에 "글을 읽을 수 있을 정도"라는 조건이 있었다. 모바일에서
6px 는 판독 불가에 가깝고 접근성 기준에도 한참 못 미친다 → **10.5px** 로 잡았다(기존 대비 27%
축소, 화면에서 확실히 작아 보이면서 읽힌다). 더 줄이려면 이 값 하나만 내리면 된다.
