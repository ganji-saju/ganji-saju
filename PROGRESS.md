...
- **후속 #614**: 방문자·PV 그래프 **바로 아래 날짜별 상세 테이블** 추가(최신순·스크롤+sticky 헤더 max-h440·컬럼 날짜/방문/PV/신규가입/결제/매출/결제·방문(참고)). 기존 `snap.daily`+포맷터 재사용, 순수 프레젠테이션(데이터 파이프라인 무변경). 차트 그리드를 (방문+PV)/테이블/(나머지5) 3블록 분리. typecheck 0·build 성공.
