...
- **홈 8캐릭터 카드를 12간지 대화선생 대화로 연결**: 홈(`GANGI_HOME_CARDS`) 8장이 '선생 캐릭터'처럼 보이나 실제 대화(/dialogue)는 '대화상담' 1장뿐이고 7장은 기능페이지 바로가기였음(사용자: "대화 가능한 것만"). 각 카드 href 를 주제 매칭 대화선생으로: 사주→/dialogue/dragon, 대운→tiger, 택일→horse, 궁합→sheep, 꿈해몽→snake, 대화상담→dog, 무료타로→rabbit, 무료운세→ox (택일·꿈·상담·운세는 zodiac 도 주제에 맞게 정정). 8장 전부 실제 대화방 진입. 사주/대운/택일/궁합 카드 price 라벨도 '9,900원'→'무료 상담'으로 통일(무료 대화 진입 반영; 해당 선생이 대화 중 9,900 리포트로 안내하는 퍼널). 8장 전부 무료 진입 표기. 참고: DALBIT_TEACHERS(moonlight)·specialist-mentors 는 별개(전자는 미사용 데이터, 후자는 리포트형 4명).
