...
- **대화 선생 명칭을 서비스명으로 통일(드롭다운·허브·인사말·부제·헤더 전부)**: 사용자 요청 — '사주용선생' 류 페르소나명 제거, 짧은 서비스명(사주/명리/길일/궁합/꿈해몽/대화상담/타로/오늘운세/별자리, 숨김 3명=성향/관상/행운)으로. `DIALOGUE_EXPERTS.teacherName` 12개 값 자체를 변경 → 드롭다운·/dialogue 허브·in-chat 인사말("안녕하세요, 사주예요…")·헤더·기록 모두 자동 통일. 인사말 받침 문법은 `withCopula`(받침 유무로 예요/이에요) 보정. 드롭다운 부제는 선생명 제거하고 동작구(내 흐름 보기 등)만. ⚠️ 예약상담 페이지(/dialogue/appointment, 출시예정)의 '명리호선생'은 별개 기능이라 유지. tsc 0.
