...
### 검증
- 유닛 1,657 + node:test 191 통과, tsc 0. 가짜 DB 는 SQL 처럼 `NULL = x` 를 거짓으로.
- 뮤테이션 12/12 red(필터 하나씩 제거·빈 결제키 가드 삭제·감사 누락·옛 회수 복귀 등). 1건은 처음에 생존했는데 테스트 구멍이 아니라 같은 문자열이 옛 함수에 먼저 있어 엉뚱한 곳을 바꾼 것 — 대상 고쳐 red.

### 남은 것
- 🔴 **1회성 정리(쓰기 — 사용자 승인 대기)**: 환불·취소 주문의 결제키로 남은 이용권 6행 + 레거시 10행을 새 코드와 같은 기준으로 삭제 + 감사 행. SQL 은 PR 본문.
- 보고만: 멤버십 환불 시 구독 해지 없음(0건 — 기간 누적 계산 얽혀 별도) · 나이스 취소가 멤버십에 지급 안 한 90전 회수 · 관리자 수동 회수의 레거시 범위 불일치 ·
  오늘 자세히 같은 사주 다른 날 재구매가 옛 행에 흡수(결제 1건뿐 — 잠재) · 단품 먼저 산 뒤 번들 구매 시 번들 몫 today-detail 당일 판정.
