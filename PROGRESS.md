...
- ⚠️ **묶음 9,900 = `taste_score_total` 9,900 동가 → 묶음이 strictly dominant** (미결, 가격정책 결정 필요).
  `score-unlock-access.ts:79` grandfather 가 묶음의 score-factor 5개를 score-total 해제로 인정하므로,
  같은 값에 묶음이 today-detail 을 얹어 준다. 단 두 CTA 가 **같은 화면에 뜨는 곳은 0곳**
  (묶음=오늘운세 페이월 / score-total=사주 점수 게이트)이라 체감 모순은 즉시 발생하지 않는다.
  권고안: score-total 9,900 → 6,600(3,300의 2배)로 서열 복원. 집행 시 costs.test.ts 동반 확인.
