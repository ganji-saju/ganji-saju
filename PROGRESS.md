...
- **검증**: 실제 DB 앵커 9/9 및 가상 명식 3종 모두 조건부 원문 연결을 확인했다. suspect·reference-only·blocked 판본과 미검수 해설이 검색에서 제외되는 트랜잭션 회귀도 통과했다. `npm test` 1,784건 + Node 193건, Vitest 66파일 497건, `typecheck`, production build, PDF layout, corpus validator, script 회귀와 `git diff --check`가 통과했다. 가상 인물로 실제 AI 기본 총평은 입력 5,944/출력 1,550 토큰, 평생은 30,178/2,182 토큰으로 생성·형식 검증했고, 오늘 유료 샘플은 빈 응답으로 결정론 폴백됐다. 해당 요청은 사용량이 반환되지 않아 실제 청구 비용은 확인할 수 없다.
