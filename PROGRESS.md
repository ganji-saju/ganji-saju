...
## 2026-09-27 — 신년운세 PDF 디자인 개편(사주팔자·오행·색 띠·여백 축소)

- 사용자 피드백(staging): PDF 여백이 많고 2027 흐름만 텍스트로 이어짐, 사주팔자 명식·오행이 빠짐 → 평생운세 PDF 처럼.
- `NewYearReportDocument` 신설 — 평생 PDF 의 rp-* A4 조각(RunningHeader·ChapterHead·DeepSection·PageFooter export)과 `buildPdfModel`(결정론: 사주팔자·오행 도넛·십성·신살)을 재사용.
  표지(명식·한 줄·오행·키워드·**12달 흐름 색 띠**) → 사주 구조(십성·신살) → 총론·분야별 8가지(자동 쪽 나눔) → 분기·월별(상/하반기 카드·분기·달별 흐름 칩) → 기대/조심 2단 + 행동 지침.
- 여백: 본문 쪽 나눔에 한도 인자 추가(`paginatePdfNarrative(…, {maxWeight,maxItems})`, 평생 기본값 불변) · 총론/분야별 한 장으로 흘림 → 쪽 채움 약 55% → 70~95%.
- 폴백 풀이 문장의 간지 한자(丁未) 한글화(명식 한자는 의도 표기).
- 검증: 신규 오프라인 회귀 `npm run test:pdf:new-year`(가짜 인물 2명·긴 본문 표본: A4 넘침 0·쪽번호 연속·물리 PDF 쪽수 일치·본문 한자 0) · `test:pdf:layout`(평생) 통과 · npm test·test:spec green · tsc 0. 쪽 캡처로 육안 확인.
...
- **종합 리포트 묶음(9,900) 구성 변경(사용자 결정 2026-09-26)**: 올해 핵심(year-core) 제거 → 4종·**16항목**(목차·홈 배너·카드·체크아웃·잠금 바·신뢰 스토리 문구 동시 변경, 가드 테스트 `comprehensive-toc.test`). 기존 묶음 구매자의 year-core 행은 유지. 가격 9,900·정가 앵커 33,000 은 그대로.
- 남은 것: staging 실결제로 전체 화면·PDF 확인 · ⚠️ 2027 입춘 정확 시각 미확인 · 홈 카드 비주얼 변경은 사용자 머지 승인(2026-09-26) · 리뷰 minor 5건(PR 본문).
...
- 실측(사용자 요청, 2026-09-25, supabase-ganji 읽기 전용): `product_entitlements` 금액>0 행 중 **같은 payment_key 2행 이상 0건 · 같은 order_id 2행 이상 0건**. 중복 합산이 실제로 일어난 사례는 없다.
- 코드는 손대지 않음 — #842 로 2행이 새로 생기지 않고 과거 잔재도 0이라 결제키 dedup 가드를 넣을 이유가 없다. 다시 생기면 같은 쿼리로 확인.

## 2026-09-25 — 의존성 묶음 업데이트 #840 머지·staging 확인

- #840(React 19.3·Next 16.3.5 등 13개) 머지 전 확인: CI·CodeQL 통과, lockfile libc 항목 삭제 없음. E2E 는 skip 이라 화면 검증은 수동.
- ✅ staging 수동 확인(사용자, 2026-09-25): 오늘운세·결제·하단 고정바 정상.

## 2026-09-23 — 당일권 날짜 scope 프로덕션 읽기 전용 확인(#841·#842)

- 사용자 승인 하에 최근 2일(KST 09-21~23) today-detail 이용권을 읽기 전용으로 조회(스크립트 `.claude/qa/check-today-detail.mjs`, 커밋 안 함 · 식별자 앞 8자만 출력).
- 결과: 1건(staging 실결제 확인과 같은 날) — scope 에 KST 날짜 박힘 ✔ · scope 날짜 = 결제일 ✔ · 주문 fulfilled ✔ · 같은 결제키 2행 **없음** ✔.
- 남은 것: 다음 날 재구매(날짜별 2행)와 자정 재시도는 표본이 없어 실데이터로는 아직 확인 못 함 — 표본 생기면 같은 스크립트로 재확인.

## 2026-09-23 — staging 401 안내 문구: "앱 계정으로는 못 들어온다"를 본문이 말하게

사용자 제보 — staging 에서 `test1111@naver.com` 으로 오늘운세를 보려다 **"로그인창이 무한 반복"**. 진단 결과 **코드 버그가 아니라 staging 접근 게이트**(#697, `src/proxy.ts` `stagingGateResponse`)였다:
- staging 의 모든 페이지(`/api/` 제외)가 `401 + WWW-Authenticate: Basic realm="ganjisaju staging"` 이라, 사용자가 본 건 앱 로그인 화면이 아니라 **브라우저 기본 인증 팝업**이다.
  게이트는 라우팅보다 먼저라 앱이 렌더되지도 않는다(`curl` 로 `/login?next=%2Ftoday-fortune` 도 401 확인). 팝업에 앱 이메일을 넣으면 영원히 401 → 팝업 반복.
- 들어가는 법: 팝업에 **아이디 아무거나 + `STAGING_ACCESS_PASSWORD`**(Vercel Preview, 설정돼 있음 확인). 브라우저가 틀린 자격증명을 기억하면 시크릿 창.
- 고친 것: 401 **본문**이 무엇을 넣어야 하는지 말하게 했다(팝업은 본문을 안 보여주지만 취소하면 이 화면이 남는다). 문구는 `staging-gate.test.ts` 가 고정(뮤테이션 red).
- 검증: tsc 0 · npm test 1790. 게이트 동작(프로덕션 미잠금·env 미설정 통과·`/api` 통과·판정 순서)은 기존 가드 그대로.
- ✅ **staging 실확인(사용자, 2026-09-23)**: 게이트 통과 후 **오늘 자세히 1건 결제 → 정상 열림**. #841(날짜별 이용권 기록)·#842(자정 재시도 1행·보관함 사주당 1장)의 배포 후 QA 통과 — 이 줄이 그 확인 기록이다.
  (자정 경계·다음 날 재구매는 실결제로 확인하지 않았다 — 스펙·뮤테이션으로만 고정돼 있다.)
...
- **바뀐 것**: 저장 scope `today:<readingKey>` → `today:<readingKey>:<KST 날짜>`. 생성은 `resolvePaymentProductScope` 한 곳뿐이라 결제 지급·번들·관리자 수동 지급이 모두 따라온다.
  `grantProductEntitlement` 의 "지난 날 행을 이 결제로 갱신" 분기와 `isEarlierKstDay` 삭제 — 다음 날 재구매는 **다른 scope** 라 새 행이 들어간다(같은 날 재지급만 멱등).
- **왜**: 어제 행을 덮어쓰면(created_at·결제키 교체) 어제 주문이 이용권 없는 결제가 된다 → ① 결제키 회수 대칭(#819) 깨짐 ② 관리자 이용권 기준 환불 화면에서 어제 구매 실종
  ③ 멤버십 환불 잠금 근거(created_at)가 흔들림. 이제 결제 1건 = 이용권 1행.
- **판정은 그대로**: "오늘 것인가" 의 정본은 여전히 행의 `created_at`(KST 하루). 새 `parseTodayDetailScopeReadingKey` 가 마지막 `:YYYY-MM-DD` 만 떼고 사주를 대조하므로
  옛 행(날짜 없음)도 그대로 읽힌다 — **마이그레이션 없음**.
- 죽은 코드 `todayDetailEntitlementScopeKeys`(+전용 테스트) 삭제: 프로덕션 호출부 0이고, 새 형식에선 정확일치 조회가 성립하지 않아 오해만 남는다.
- 검증: regrant 스펙 재작성(다음 날 = 2행·어제 결제키/주문 보존·어제 기준으로도 열림 · 옛 형식 행과 공존) · 판정 +3(날짜 scope 대조·created_at 정본·readingKey 에 ':' 없음)
  · scope +2(저장 형식·KST 자정 경계) · 파서 계약 +1. 뮤테이션 3종(저장 날짜 제거 · 파서 무력화 · 당일권 정확-scope 조회 무력화) 전부 red. tsc 0 · npm test 1787 · test:spec 498.
- ⚠️ 실측(2026-09-23): 지금은 파서를 빼도 **판정 결과는 같다** — `fromSlug` 가 꼬리 토큰을 무시하고, 해석 실패 행은 레거시 규칙이 어차피 연다. 그래서 파서는 저장 형식의 계약으로만 고정했다.
- 남은 것(리뷰 이월): 재구매 뒤 어제/오늘 **결제키 회수 실호출** 단언(지금은 행·결제키 보존으로 간접 확인) · prepare 가 판정에 넘기는 인자 단언.

## 2026-09-15 — 관리자 환불 동시 승인 이중 환불 차단(승인 선점)

브랜치 `fix/refund-approve-claim` — #829 머지 전 리뷰의 후속(중, 머지 비차단으로 판정됐던 것).
- **버그**: `executeRefund` 가 읽어 둔 스냅샷 상태만 보고 `setStatus(processing)` 을 **조건 없이** 덮어썼다. 두 탭·두 관리자가 같은 요청을 거의 동시에 승인하면
  둘 다 requested 를 읽고 PG 취소를 한 번씩 보냈다 — 나이스 취소는 호출마다 새 orderId 라 **일부 취소는 두 번째도 수락**(이중 환불). 전 부분환불(credit_purchase)에
  원래 있던 경합이 #829 로 멤버십 일부 환불에도 열렸다. 같은 탭 연타는 busy 잠금이 막아 동시 승인일 때만(창 = 조회→전이 사이 DB 왕복 1회).
- **수정**: `setStatus(…, { from })` = 선점 — `.eq('status', from)` 조건부 update 후 바뀐 행이 없거나 오류면 false → **PG 취소 전에** "다른 승인이 이미 처리 중" 으로 멈춘다.
  승인(requested/failed)·revoke_pending 재시도 두 곳. 그 밖의 전이는 예전처럼 결과를 보지 않는다(완료 기록 실패로 PG 성공 뒤 500 이 나지 않게).
...
- **관리자 해제**: `cancelled` 는 renews_at 까지 혜택이 남고(isEntitledStatus) 사용자가 재개할 수 있어 해제가 안 됐다 →
  `expireMembershipNow` = `expired` + renews_at=지금(남기면 재구매 때 activate 의 base 로 되살아남).
- **대화상담 질문 3회 취소**: 카탈로그 credits=0 인데 전 3개를 줘서 나이스 취소 회수가 0이었다 → `DIALOGUE_QUESTION_CREDITS` 상수를 지급·회수가 공유,
  `creditsToRevokeOnCancel`(멤버십 0 · 대화상담 3 · 전 충전은 카탈로그). 관리자 환불은 원래 lot 회수라 이미 대칭.
- 검증: 유닛 1,669 + node:test 191, tsc 0 · 뮤테이션 6/6 red.
- 대기: 3번(멤버십으로 연 달력·상세의 환불 후 열람) 정책 해석 확인 · 1번(나이스 취소 응답 확인용 테스트 결제) 계획 승인.

## 2026-09-13 — 멤버십 환불 시 구독 종료 (+ 결제키 회수 1회성 정리 실행)

앞 섹션 #819 머지·staging 동기화 완료(`291c5dfa`). 마이그레이션 없음.

### 1회성 정리(사용자 승인, MCP 한 문장 실행)
환불·취소 주문의 결제키로 남은 권한 이용권 6 + 레거시 10 → **0**, 보존 33(기존 감사 30 + 원장 3) 그대로, 감사 14행(`reason='cleanup-2026-09-13-refunded-orders'`). 실행 전후 읽기 전용 검증.

### 왜
관리자 환불·나이스 취소가 멤버십을 환불해도 구독이 안 끝났다(이용권 0행 → nothingToRevoke → completed, renews_at 그대로). 나이스 취소는 지급 안 한 90전을 회수하려 했다(카탈로그 credits=90, 코인 sunset 뒤 멤버십은 전 미지급).
프로덕션: 유료 멤버십 결제 0건 · 구독 5행(관리자 부여로 추정, 4행이 30일 넘게 남음) → **지금 종료하면 관리자 부여 기간까지 날아간다**.

### 방식(조사 워크플로 2렌즈 → 리뷰 반영)
- 지급이 구독에 **실제로 더한 일수**를 주문 metadata(`membershipDaysGranted`)에 누적 기록하고, 환불은 **그 값만** 뺀다(`membershipDaysToRemove`).
  처음엔 "항상 30일"을 뺐는데 리뷰가 잡았다 — 승인만 되고 지급 전 실패한 주문은 받은 적 없는 30일을 관리자 부여분에서 깎고, 지급 재시도(+60)는 30일이 남는다.
  기록이 없으면 0(유료 멤버십 결제가 0건이라 옛 주문 문제 없음).
- 뺀 결과가 지금 이전이면 `expired` + renews_at=지금(남기면 재구매 때 되살아남). 상태만 cancelled 는 혜택이 안 끊겨서 안 씀. renews_at 없음(무기한)은 손대지 않음.
- 훅은 `markPaymentOrderRefunded` 의 **방금 refunded 로 바뀐 분기** — 관리자·웹훅·정산이 겹쳐도 조건부 UPDATE 로 정확히 1회. 차감 실패는 삼키지 않고 주문 `last_error` 에 흔적.
- **부분취소는 구독 유지**(관리자 부분환불과 같은 결과 — 전엔 웹훅 partialCancelled 가 30일을 전부 뺄 뻔했다). 정책 변경 가능.
- 나이스 취소의 전 회수에서 멤버십(구독)만 0 — `shouldGrantCredits`(영구 false)로 막으면 옛 전 충전 주문 회수까지 꺼져서 구독만 뺐다.

### 검증
유닛 1,666 + node:test 191, tsc 0 · 뮤테이션 10/10 red(리뷰가 생존 실측한 멱등 분기 차감·실패 삼킴·update 필터 컬럼 오류 포함).

### 남은 것(보고)
관리자 "멤버십 해제"가 cancelled 라 혜택이 안 끊김(같은 원인) · 차감(select→update)과 같은 순간의 재구매가 겹치면 한쪽 갱신 소실(원자 RPC 는 마이그레이션) ·
멤버 경로로 이미 연 달력·상세 기록은 환불 뒤에도 열람(정책 판단) · 대화상담 질문 3회는 전 3개를 주는데 취소 시 회수 안 함(pkg.credits=0) ·
🔴 나이스 취소 웹훅 위조(기존 Medium) — 이제 멤버십 기간 차감까지 영향. 응답 스키마 확인 후 재조회 가드.

## 2026-09-13 — 환불·PG 취소 회수를 결제키 기준으로 (그 결제가 만든 권한만)

앞 섹션 PR7(#818)은 머지·staging 동기화 완료(`04079646`, main=staging). 이번 PR 도 마이그레이션 없음.

### 왜 (코드 조사 3렌즈 + 프로덕션 읽기 전용 집계 — 사용자 승인)
회수가 (user, product, scope) 로 지웠다. `product_entitlements` 는 이 셋이 유니크라 행을 **먼저 산 주문**이 차지하고, 뒤 결제는 grant 가 기존 행을 돌려받아 자기 행이 없다.
- 번들 환불이 따로 산 같은 이용권·무료 지급까지 지운다(코드상 도달 가능, 현재 판매 번들 6건 중 해당 0 — 잠재).
- 주문 단위 환불(고아·멤버십)은 요청 `product_id` 가 **패키지 id** 라 아무것도 못 지운다(감사 행 5건 실측 — 이용권이 이미 없던 주문이라 무해).
- 🔴 전역 상품(재물·직장 주제)은 레거시 `scopeKey`(null) ≠ 회수 필터('global') → **환불 뒤에도 열람이 되살아났다**. 번들 환불 경로가 재계산한 scope 로 지워 해가 바뀌면 원래 행을 놓친다.
- 실측: 환불·취소 주문인데 남은 권한 **이용권 6행(1명, 취소된 번들) + 레거시 10행(4명)**. 결제 이용권 51행은 전부 결제키 보유·결제키 공유 0 → 결제키 기준이 데이터상 성립.

### 방식
- `revokeEntitlementsOfPayment(userId, paymentKey)` — 이용권 행(user, payment_key) + 레거시 grant 행(user, type=purchase, feature∈taste_product·lifetime_report, metadata ∋ paymentKey)을 지우고 지운 것마다 감사 행.
  결제키·사용자가 비면 던진다(넓게 지우는 경로 없음). feature 필터가 핵심 — 실측상 같은 결제키에 **보존해야 할 전 원장·감사 행 33개**가 붙어 있다.
- 관리자 환불(단품·번들·고아·평생리포트)과 나이스 취소 웹훅이 이 함수 하나를 쓴다. 취소 계획은 "지급된 상태인가"(`revokeGrants`)만 판단.
- 호출부가 사라진 번들 재계산 회수(`revokeBundleEntitlement`·`revokeBundleComponents`)·주문 열거(`listProductEntitlementsByOrder`)·평생리포트 회수 래퍼 삭제.
  `revokeProductEntitlement` 는 관리자 수동 부여 회수 전용으로 남김(가드 테스트가 호출부 고정).
- 환불 목록: PR7 의 "금액 없는 단품 이용권 주문은 뺀다" 우회를 걷어내고 "목록에 올라간 것 = 잡힘"으로(실행이 결제키로 회수하므로 번들·단품 구분 불필요).

- 독립 리뷰 + 적대적 검증(격리 worktree, postgrest-js 소스로 요청 모양 실측 — `cs.{"paymentKey":…}`·DELETE 비재시도·0행 `[]`) 반영:
  감사 = 이용권 행 ∪ 이용권 행이 없던 (상품, 정규화 범위)의 레거시(흡수 구성품 감사 누락·평생리포트 형식 어긋남) ·
  웹훅 회수 실패를 `processed` 대신 `failed` 로(주문은 이미 refunded 라 재통보가 회수를 안 해 추적이 끊겼다) ·
  이용권 회수 게이트를 지급이 **일어났을 수 있는** 상태로(`fulfillment_failed` — 구성품 일부만 지급된 주문의 남은 권한).

### 검증
- 유닛 1,659 + node:test 191 통과, tsc 0. 가짜 DB 는 SQL 처럼 `NULL = x` 를 거짓으로, 단계별 오류 주입.
- 뮤테이션 24/24 red(필터 하나씩 제거·빈 결제키 가드·오류 삼킴·감사 실패 throw·웹훅 게이트/try/failed 표기·지급 실패 제외 등).
  두 번은 처음에 "생존"했는데 테스트 구멍이 아니라 뮤테이션 대상 문자열 문제(옛 함수에 같은 문자열이 먼저 있음 · 코드가 바뀌어 문자열 없음) — 고쳐 red.

### 남은 것
- 🔴 **1회성 정리(쓰기 — 사용자 승인 대기)**: 환불·취소 주문의 결제키로 남은 이용권 6행 + 레거시 10행을 새 코드와 같은 기준·감사 규칙으로 삭제(감사 14행).
  SQL 은 PR 본문, 같은 로직의 읽기 전용 미리보기로 6·10·14 확인.
- 🔴 보안(기존·Medium): 나이스 취소 웹훅에 서명 검증이 없고 PG 재조회 결과를 진위 판정에 안 쓴다(재조회 키도 통보 본문 tid) — 위조 통보로 자기 주문을 refunded 표기·권한 회수 가능.
  재조회를 `order.paymentKey` 로 하고 "조회 성공 + 취소 아님"이면 무시하는 가드가 답이지만, 나이스 응답 스키마가 미확인(코드 주석 "스캐폴드")이라 정상 취소를 무시할 위험 → 응답 확인 후 별도.
- 보고만: 흡수 결제의 소유 모델(먼저 산 쪽 환불 시 뒤 결제는 레거시로만 남아 product 행만 보는 판정에서 잠김 — 결제별 행 허용은 마이그레이션) ·
  지급 중 환불 경합(`markPaymentOrderFulfilled` 상태 가드 없음) · 토스 웹훅·정산 크론 취소 감지에 회수 없음 · 평생리포트 2회 결제 시 두 번째 결제 흔적 없음 ·
  멤버십 환불 시 구독 해지 없음(0건) · 나이스 취소가 멤버십에 지급 안 한 90전 회수 · 관리자 수동 회수의 레거시 범위 불일치 · 오늘 자세히 재구매 흡수(결제 1건뿐).

## 2026-09-13 — 할인쿠폰 PR7: 설계 §13 가드 보강 + 번들 환불·결제내역 금액 버그

앞 섹션의 다듬기 PR #817 은 머지·staging 동기화 완료(`f6b01612`, main=staging). 이번 PR 도 마이그레이션 없음.

### 왜
감사 워크플로(3렌즈 — 환불 금액 경로 · 가격 표시 맵 · §13 커버리지)로 보니 §13 가드 10개 중 **강함 3 · 약함 6 · 없음 1**(§13-9 환불).
main 의 옛 테스트에 뮤테이션 31개를 돌리면 **1개만 red** — 문자열 정규식이 큰따옴표·별칭 import·구조분해·다른 파일을 못 보고,
픽스처 금액이 카탈로그가와 같아 "정가로 환불"이 초록으로 지나갔다. 즉 28개 회귀 경로가 CI 를 통과할 수 있었다.

### 방식
- 소스 스캔(`coupon-chokepoint.test.ts`): 초크포인트(따옴표 무관·upsert·별칭·`AUTHNICE` 직접 호출) · prepare **성공 응답 객체 안** 금액 ·
  요청 키 **허용 목록**(+쿼리스트링 금지) · 멈춤 분기 3곳 · 가격 표시 체인 4파일 + 루트 레이아웃 정적 + `<PriceProvider>` 1곳 ·
  환불 요청 스냅샷 · 지급 금액 `claimed.amount`(단축 속성 포함) · 정가 필드 사용처 화이트리스트 · 주문 조회 `select('*')` · 주문 금액 UPDATE 금지 ·
  전 차감·언락 경로의 할인쿠폰 import 금지 · 체크아웃 금액(버튼·요약·GA 값) = `chargeAmount`.
- 행동 테스트: 환불 목록(카탈로그가와 **다른** 할인가) · §13-8 결제 완료가 쌓인 3건 연속 할인 · 절사 끝수(999.9) · 읽은 주문의 `couponCode` ·
  회수 조회 실패 = 주문 있음 · 결제내역 금액 우선순위(원장 > metadata > 카탈로그).
- 정규식은 오탐을 줄였다: 줄 끝 주석 제거 · 사용자와 무관한 이름(`useReducer`·`sessionStorage`) 통과 · 포매터 줄바꿈 허용 · 정적 ISR 허용 · 토스 **타입** import 허용.
- 버그(모두 번들 종합 리포트 — 판매 중·쿠폰 대상):
  - 관리자 환불 목록에서 **구성품이 지급된 번들 주문이 통째로 빠짐**(관리자 화면으로 환불 불가) — 2026-08-27 고아 주문 수정이 구성품의 주문번호까지
    "이미 잡힘"으로 셌다. 번들 주문만 중복 판정에서 뺐다. 금액 없는 **단품** 이용권의 주문은 일부러 종전대로 둔다 — 주문 단위 환불은 요청 `product_id` 에
    패키지 id 를 실어 단품 이용권을 회수하지 못한다(넓게 고치면 "환불됐는데 열람이 남는" 경로가 열린다 — 처음엔 넓게 고쳤다가 되돌림).
  - 사용자 목록 요약(`summary-refresh`)이 주문 원장을 안 넘겨 "환불 가능" 필터·배지가 상세 화면과 달랐다 → 인자를 **필수**로(빠뜨리면 타입 오류 — 실측).
  - 할인된 번들이 결제내역·관리자 LTV 에 **카탈로그 정가 9,900원**으로 계상 → 금액 없는 carrier 는 주문 원장 금액 우선(DB 행은 안 건드림 — 설계 §9 경고와 무관).

### 검증
- 유닛 1,659 + node:test 191 통과, tsc 0.
- 뮤테이션 44개(적대적 검증이 격리 worktree 에서 찾은 생존 회귀 12개 포함) **44/44 red** · 정상 변경 6개 **오탐 0** · 작업트리 해시로 원복 확인.
- 독립 리뷰(pr-reviewer 형식) + 적대적 검증 2에이전트. 반영: 위 12개 · summary-refresh · 정규식 과대/과소 · 결제내역 우선순위 테스트.

### 보고만 한 것(미수정 — 결정 필요)
- 🔴 **번들 환불이 따로 산 같은 이용권까지 회수할 수 있다**(추정·코드 판독): `revokeProductEntitlement` 가 (사용자, 상품, scope) 로 지우고,
  이미 같은 이용권이 있으면 번들 지급이 새 행을 안 만든다. 기존 동작이지만 이번에 번들 환불 창구를 다시 열어 노출이 넓어졌다.
- 금액 없는 단품 이용권이 남은 결제 주문은 관리자 화면 **양쪽 창구가 다 막혀 있다**(이용권 경로 amount=null 거부 · 주문 경로 목록 제외). 근본 해결은
  환불 라우트가 패키지 → 이용권 id·scope 를 풀어 회수하는 것(주문 = 유일한 환불 단위). ⚠️ 해당 행 수는 프로덕션 집계로만 알 수 있다(DB 미조회).
- 전 결제 환불 실행 경로가 원장 금액을 안 넘김(`admin/refund/route.ts:75` `buildCreditRefundItem` 4번째 인자) — 옛 행은 카탈로그 정가로 기록(쿠폰 무관, §9 잔여).
- 같은 사용자가 새 코드로 prepare 를 **동시에** 2번 → 두 번째 409(돈 손실 없음, 재시도로 성공).
- 페이월의 "쿠폰은 현금 결제에만" 안내 없음 ⚠️ 검증필요(체크아웃엔 있음) · §15 운영 메일 알림(`alertOps`) 테스트 없음(설계상 '코드' 강제).

## 2026-09-13 — 할인쿠폰 staging 실확인 + 체크아웃 다듬기 2건

PR5(#815)·PR6(#816) 머지·프로덕션 배포 완료(`0279eb4a`, main=staging). 085 적용 확인(카탈로그 조회 — 접두=등급 CHECK 존재, anon·authenticated 쿠폰·감사 테이블 SELECT 불가).
머지는 auto mode 가 "Production Deploy" 로 막아 사용자가 `! ./scripts/gh-ganji pr merge` 로 실행했다.

### staging 실확인 (사용자 브라우저, super_admin)
발급 staging-test 10등급 5장(무작위 번호) → CSV 5줄·화면 DOM 에 코드 없음 → 체크아웃 입력칸에 코드 → **3,300 → -330 → 2,970원**(PR5 로그인 상태 첫 확인) →
배치 회수 → 체크아웃 3,300원 + 거부 문구 → 되살리기 → 다시 2,970원 → 해제 조회(마스킹 `ganji-10-**39 · 미사용 · 미귀속`, 입력칸 비움).
실제 해제는 안 했다(귀속은 결제 버튼에서 일어나 결제 흐름에 들어가야 함 — 유닛 테스트로만 검증). staging-test 쿠폰 5장은 살아 있다(운영에선 거부).

### 다듬기 (staging 에서 본 것)
- 회수·중지(disabled) 쿠폰 전용 안내 "회수되었거나 사용이 중지된 쿠폰입니다." — 전엔 없는 코드와 같은 "사용할 수 없는 쿠폰 코드"라 오타로 오해.
- 미리보기 쿠폰(아직 결제 전·귀속 전)은 접힌 **"다른 쿠폰 코드 쓰기"**로 바꿀 수 있다(`checkoutCouponInputMode`). 전엔 적용되면 입력칸이 사라져 쿠키 30분 동안 못 바꿨다.
  등록된 쿠폰(self)은 그대로 입력칸 없음(동시에 1개).

### 다음 세션
PR7 — CI 가드 정리(설계 §13-9 할인 환불액 = order.amount, §13-6 가격 표시 맵 가드 대상 보강).
...
- 뮤테이션 24개(인라인 — 리뷰 워크플로 4에이전트는 세션 한도로 전멸): 16 RED, 생존 8 중 **실제 공백 5개를 테스트로 막고 재실행 RED**
  (해제 재찍기 · 다른 등급 접두 · pickSerials 가 난수 무시 · 발급 기본 난수 순차 · 내보내기 감사 `.catch` 삼키기). 나머지: #12 는 동작 불변(무효), #23 은 이중 방어의 두 번째 겹(쿼리 필터가 먼저 막음).
...
- 결제 금액 카드에 **접힌** "쿠폰이 있으신가요?"(`<details>`) — 펼쳐 두면 쿠폰이 없는 사람이 쿠폰을 찾으러 이탈한다.
  넣은 코드가 안 붙었거나(사유 문구) 등록된 쿠폰이 죽었으면 펼친 채로 그린다. 입력칸은 **결제 버튼이 있는 화면 · 쿠폰이 붙는 상품(설계 §7) ·
  적용된 쿠폰 없음**일 때만(적용 중이면 할인 행이 곧 표시 — 동시에 1개라 새 코드는 어차피 거부된다).
- 입력은 **서버 액션**(`src/app/membership/checkout/coupon-action.ts`) → 체크아웃 전용 쿠키 `ganji_coupon_input`
  (httpOnly · lax · path=/membership/checkout · 30분). GET 폼이면 코드가 URL 에 실려 방문기록·GA·리퍼러로 새고, 다른 사이트가 연 링크가
  사용자의 조회 예산을 태운다. 서버 액션은 POST 이고 Next 가 Origin 을 검사한다. 쿠키를 바꾸면 Next 가 같은 화면을 다시 그리므로 리다이렉트가 없다.
  액션은 판정하지 않는다 — 코드 검사·조회 예산은 렌더의 `resolveChargeForUser` 한 곳(두 번째 입구를 만들지 않는다).
- 체크아웃은 **쿠키 → `?coupon=`** 순으로 읽는다(액션이 URL 을 안 바꿔서, QR 로 들어와 다른 코드를 넣으면 URL 의 옛 코드가 이기면 안 된다).
  `?coupon=` 은 QR·앱 링크용으로 남기고 교차 사이트 가드도 그대로.
- 비로그인은 조회하지 않아 사유 문구가 없다 → "로그인하면 넣은 쿠폰이 적용돼요" 안내. 쿠키가 30분·lax 라 결제 버튼 → 로그인 → 복귀 뒤에도 적용된다.
- "쿠폰은 이 화면의 카드·간편결제에만 적용돼요. 전으로 여는 경우엔 적용되지 않아요."(설계 §7).
- 입력칸 제출 재렌더는 `checkout_viewed` 를 다시 남기지 않는다(`next-action` 헤더) — 퍼널은 건수 집계라 제출마다 분모가 부푼다.

### 검증

유닛 전체 통과(가드 4개 추가 — 서버 액션·쿠키 옵션·GET 폼 금지 · 쿠키 우선 + 교차 사이트 가드 유지 · 재렌더 퍼널 미기록 · 제외 상품). tsc 는 `.next/types` 잔재 1건 외 없음.
로컬 dev 에서 실제 조작: 접힘 → 펼침 → 제출 = 같은 URL 로 POST(주소창에 코드 없음) · 쿠키 JS 비노출 · 새 GET 에도 코드 유지·펼침 ·
빈 값 제출로 쿠키 삭제·다시 접힘 · 390px 폭 가로넘침 없음 · 임시 로그로 `next-action` 이 GET=false / 액션 재렌더=true 확인(로그 제거).
⚠️ 로그인 상태의 실제 할인 적용은 로컬에서 안 봤다(로컬=프로덕션 DB 라 조회 예산을 쓴다) — 계산 경로는 PR3 그대로(`resolveChargeForUser`)이고 입력 출처만 바뀌었다.
...
- 재검증(4건 닫힘) 뒤 Low 3건 반영: 안내를 "**이번 요청으로** 청구된 금액은 없습니다"로(환불된 주문·나이스 승인 타임아웃처럼 이전에 돈이 움직였을 수 있음) ·
  정산 관문을 IN_PROGRESS 만이 아니라 **DONE·종료 상태가 아닌 모든 경우**로(READY 에서 관문 없이 키가 붙으면 뒤따르는 confirm 이 쿠폰 검사를 건너뛴다).
...
합산 상한이 없었다 — 체크아웃 `?coupon=` 미리보기가 무료 계정 수천 개로 하루 안에 전단 코드 전량을 수확하는 오라클이었다.

### 무엇을 했나

- 081: `rate_counters(bucket, period_key, used_count)` + 원자 RPC `consume_rate_counter`(한도 안이면 +1 값, 닿았으면 NULL).
  056 은 `user_id` 가 auth.users FK 라 "모든 계정 합산" 버킷을 못 담는다.
- 새 코드 조회 **전에** 예산을 쓴다(적중 포함): 주체(카카오·구글 신원, 없으면 계정)당 하루 10 → 등급 풀.
  등급 = paid(프로덕션 fulfilled 결제) · social(`user.identities[]` 카카오·구글) · open(나머지), 풀 20/40/30(env `COUPON_POOL_*`).
- 🔴 거부는 조회 전이라 코드가 유효하든 아니든 **같은 답**이다 — 한도 뒤에 "맞는 코드만 통과"를 두면 오라클이 다시 열린다.
- 🔴 **실패-닫힘**: RPC 오류·null 이면 새 코드 조회만 막힌다(결제·등록 쿠폰은 이 경로를 안 탄다). PR3 의 056 경로(fail-open 헬퍼)는 제거.
- 풀 소진은 `busy`("지금은 쿠폰 확인이 몰려…") — 이 사람 탓인 `rate_limited` 와 문구를 분리. 풀 50%·100% 에 운영 메일 1회씩
  (실제 프로덕션 배포에서만 — 테스트가 `env:'production'` 으로 돌 때 로컬의 Resend 키로 실메일이 나가지 않게).
- `resolveChargeForUser` 두 번째 인자를 `userId` → `getUser()` 의 user 객체로. 체크아웃·prepare 가 같은 등급을 내야 화면·청구가 같다.
  타입이 두 호출부를 강제로 맞추게 했다.
- 익명 로그인(`is_anonymous`)은 조회 자체를 안 한다(지금은 꺼져 있지만 켜지는 순간 무료 계정 공장).

### 믿을 수 있는 신호만 쓴다

`user_metadata`(카카오 해시를 뽑던 경로)는 사용자가 `updateUser({data})` 한 줄로 덮어쓴다 — 등급 판정에 쓰지 않는다.
`identities[]` 의 `id` 는 제공자 사용자 ID(`identity_id` 가 행 uuid)라 탈퇴·재가입해도 같은 주체가 된다.
결제 등급은 staging 샌드박스(같은 DB)를 빼려고 `isRealRevenueOrder`(기존 헬퍼) 로 origin 을 거른다.

### 적대적 리뷰(2관점 + 반박 검증) — 확인 6건 전부 반영

| # | 문제 | 수정 |
|---|---|---|
| 🟠 | 체크아웃은 탭·앱 복귀 때마다 헤더의 `router.refresh()` 로 다시 렌더된다 → **매 렌더 과금**이라 정상 고객이 코드 하나로 한도에 걸린다. 내 테스트는 호출 3번만 흉내 낸 **거짓 green** | 같은 (주체, 코드)는 KST 하루 1회만 과금. 표시는 **과금 성공 뒤에만**(거부된 질문을 표시하면 두 번째 질문이 공짜) |
| | 세션 쿠키가 SameSite=Lax 라 교차 사이트 `?coupon=` 링크로 남의 예산을 태운다 | `Sec-Fetch-Site: cross-site` 면 미리보기 안 함(QR·앱 링크·직접 입력은 그대로) |
| | staging 경로는 예산이 따로라, 실물 코드의 만료 문구·보유자 조회 시간으로 존재가 샌다 | 환경 불일치 행은 조회 직후 not_found 로 끝냄 |
| | paid 판정이 origin `unknown`(허용목록 밖 호스트) 새 주문을 인정 | production 또는 origin 기록 없는 옛 주문만 |
| | `COUPON_POOL_*` 를 비우면 `Number('')=0` 이라 풀이 닫힘 | 빈 값은 기본값, '0' 명시만 닫힘 |
| | 운영 메일을 요청 경로에서 await | `after()` 로 응답 뒤 발송 |

### 검증

유닛 1,573 통과 · typecheck. 뮤테이션 14/14 red — 주체 한도 제거 · 조회 후 차감(오라클 재개방) · email identity 를 소셜로 ·
결제 origin 필터 제거 · 풀 하나(S1) · 버킷에 env 없음 · 카운터 실패-열림 · 익명 가드 제거 · 리뷰 수정 6건 되돌림.
(빈 env 테스트는 처음에 "N번 뒤 busy"만 봐서 처음부터 닫힌 풀도 통과했다 — 첫 조회 통과까지 단언하게 고쳤다.)
081 은 프로덕션 적용 확인: RPC 가 한도 2 에서 1 → 2 → NULL, 한도 0 에서 NULL(검증 행은 삭제).

### ⚠️ 적용 순서

**080 → 081 을 프로덕션에 먼저 적용한 뒤 머지**(#807 다음). 081 없이 배포되면 RPC 가 없어 새 코드 조회가 전부 거부된다
(실패-닫힘 — 결제·등록 쿠폰 영향 없음). **PR6(발급 화면)보다 먼저** 들어가야 한다 — 발급 전이면 오라클은 비어 있다.

## 2026-09-11 — 할인쿠폰: 요구 6 을 "평생 1개" → **"동시에 1개"** 로 (migration 080)

사용자 결정. PR3(#806) 리뷰에서 리뷰어 2명이 독립적으로 지목한 문제를 푼다.

### 문제

079 의 부분 유니크 인덱스는 `where bound_user_id is not null` 뿐이라 **만료·회수된 쿠폰도 자리를 계속 차지**했다.
그러면 설계가 권하는 실무 대응 — 전단이 빨리 소진되면 그 배치를 `disabled_at` 으로 끄고 재발행 — 에서
**끈 배치의 정상 고객은 재발행 코드를 영구히 못 쓴다.** 2027-12-31 만료 뒤 다음 캠페인에서도 기존 사용자 전원이 빠진다.
화면은 "이미 다른 쿠폰이 등록되어 있어요"라고 하는데 그 쿠폰은 죽어 있고 보이지도 않는다.

### 방식

- 080: `released_at` 컬럼 + 인덱스를 `where bound_user_id is not null and released_at is null` 로 교체.
- 죽은 쿠폰(만료·회수·등급 회수)을 가진 계정이 새 코드를 넣으면 받아 준다. prepare 가 귀속할 때 **먼저** 옛 행에
  `released_at` 을 찍고(순서가 반대면 새 귀속이 유니크에 걸린다) 새 코드를 귀속한다.
- **행을 지우거나 `bound_user_id` 를 비우지 않는다.** 비우면 관리자가 배치를 되살리는 순간 이미 쓴 전단 코드를
  남이 다시 귀속할 수 있다(요구 4 위반). `released_at` 은 **종료 상태** — 되살려도 누구에게도 적용되지 않는다.
- 🔴 **환경 불일치는 "죽음"이 아니다.** staging 에서 실물 쿠폰을 죽었다고 보고 비우면 staging 이 프로덕션 고객의
  쿠폰을 지운다(같은 DB). 판정 기준은 `couponDeadReason` 한 곳.

### 적대적 리뷰(2관점 + 반박 검증) — 2건 확인, 둘 다 반영

| # | 문제 | 수정 |
|---|---|---|
| 1 | 옛 쿠폰 release 직후 새 귀속이 경합에서 지면 되돌리지 않아 **계정 쿠폰 0개** — "죽음"은 일시적일 수 있어(관리자가 배치를 되살림) 본인 쿠폰을 영구히 잃는다. 내 주석 "잃는 것이 없다"가 틀렸다 | 새 귀속 실패 시 **이번 요청이 찍은 released_at 만** 되돌리는 보상 UPDATE |
| 2 | staging 이 **죽은 실물** 쿠폰은 비울 수 있었다(살아 있는 것만 막혀 있었다). 기존 테스트는 살아 있는 쿠폰만 시드한 **거짓 green** | `canReleaseCoupon`: 운영=모든 죽은 쿠폰, staging=staging-test 행만 |

같이 고친 PR3 결함(합산 상한 조사 중 발견): 추측 시도 제한이 **실패-열림**이었다. 공유 헬퍼
`getMemberBenefitUsed` 는 주석엔 "오류 시 limit 으로 간주"라고 적혀 있고 실제로는 0 을 돌려준다 — RPC 장애 동안
추측 제한이 조용히 사라진다. 헬퍼는 무료 메뉴(실패-열림이 맞음)도 쓰므로 건드리지 않고, 쿠폰만 RPC 를 직접 불러
오류면 `rate_limited`(새 코드 조회만 막힘, 결제·등록 쿠폰은 그대로).

테스트용 가짜 DB 의 유니크 흉내도 고쳤다 — "새 귀속"만 검사해서, 보상 UPDATE 가 자리를 이중 점유하는 걸 못 봤다
(실제 부분 유니크 인덱스는 **모든** UPDATE 뒤 상태를 본다). 이걸 고치자 기존 경합 테스트가 제대로 red→green 을 탔다.

### 검증

유닛 1,564 통과(+11) · typecheck · 마이그레이션 번호 감사 0건. 뮤테이션 9/9 red — 079 동작 복귀 · release 생략 ·
귀속 조회에 released 포함 · 환경 불일치를 죽음으로 · claim CAS 에서 released 조건 제거 · released 를 종료 상태로 안 봄 ·
리뷰 수정 2건 되돌림 · 시도 카운터 실패-열림 복귀.
못 잡힌 것 2개와 처리: "self 에도 releaseCode" 는 도달 불가 → 가드 삭제. "보상 UPDATE 의 `released_at = 이번 시각` 조건 제거"는
모든 경합 경로를 부분 유니크 인덱스가 먼저 막아 관찰되지 않는다 → **남긴다**(PR6 에서 관리자가 released 를 찍는 흐름이
생기면, 이 조건이 없으면 사용자 요청의 보상이 관리자 조치를 되돌린다).

### 같이 낸 제안서 — 전체 계정 합산 상한과 계정 확인 (`docs/coupon-lookup-cap-proposal.md`)

사용자 질문 "합산 상한을 두려면 계정 확인을 어떻게?"에 대한 답. 조사 2개 + 공격모델 설계 1개.
**결론: 합산 상한은 계정 확인이 아니라 전역 카운터로 만든다. 계정 확인은 "어느 풀에서 차감하느냐"를 가르는 데 쓴다**
(공용 풀 하나면 무료 계정 4개로 전 고객을 막는 DoS 가 된다). 추천 S2 = 위조 불가 신호(`user.identities[]` 소셜 신원 ·
프로덕션 fulfilled 결제)로 3등급 풀. 무료 계정 공격자 누설 하루 0.2~1장(지금은 하루 안에 전량).
🔴 조사 중 `user_metadata`(카카오 해시 추출 경로)가 **사용자가 직접 덮어쓸 수 있는 값**임을 확인 — 신뢰 신호로 쓰면 안 된다.
🔴 별건: `/api/auth/confirm-email` 이 **무인증으로 아무 이메일이나 강제 확인**(코드 확인) — 열거 오라클·증폭, 그리고
구글 자동 연결을 통한 선점 가입 탈취 가능성(⚠️ 재현 확인 필요). 쿠폰과 별개로 먼저 다뤄야 할 수 있다.

### ⚠️ 적용 순서

**080 을 프로덕션에 먼저 적용한 뒤 머지한다.** 순서가 뒤집히면 `released_at` 조회가 실패해 쿠폰이 전부 꺼진다
(실패-닫힘이라 금액은 안 틀리지만, 로그인 사용자의 체크아웃마다 조회 오류 로그가 쌓인다). 발급된 쿠폰은 아직 0개.

## 2026-09-11 — 할인쿠폰 PR3: prepare 귀속·할인 + 체크아웃 표시 + 폴백 3곳 제거 (돈에 닿는 단계)

설계 #800 의 구현 3단계. **이번 작업에서 처음으로 실제 청구액이 바뀔 수 있는 단계**다.
단, 발급된 쿠폰이 아직 0개(발급 화면은 PR6)라 **머지해도 사용자가 겪는 동작은 종전과 같다.**

### 한 함수가 화면과 청구를 같이 낸다

`resolveChargeForUser(pkg, userId, couponInput, { isProduction })` 를 체크아웃 화면과 prepare 가
**둘 다** 부른다. 화면 최종 금액 = prepare 의 `order.amount` = PG 청구액. 둘이 따로 계산하던 구조였다면
"화면은 할인가, 청구는 정가"가 반드시 난다. 테스트가 4개 상품(단품·멤버십·묶음·보관형)에서
`quote.chargeAmount === createPaymentOrder 가 넣는 amount` 를 고정한다.

- 쿠폰은 **상태**다. 코드를 안 넣어도 계정에 귀속된 쿠폰이 붙는다(요구 7).
- 계정당 1개(요구 6): 쿠폰이 있는 계정이 다른 코드를 넣으면 넣은 코드는 거부하고 기존 쿠폰을 계속 적용.
- 화면이 할인을 보여 줬으면 그 코드를 prepare 로 되돌려 보내고, prepare 에서 그 코드가 안 먹히면
  **정가로 조용히 진행하지 않고 409 로 멈춘다**(폴백 제거와 같은 원칙).

### 설계 원안 SQL(§5-2)에 결함 2개 — 구현하며 정정

1. **회수를 막는 주문 상태 목록에 `fulfilled` 가 없었다.** 그대로면 결제를 마친 사람의 쿠폰이
   24시간 뒤 남에게 넘어간다(요구 4 위반). → 목록을 뒤집어 "돈이 안 움직인 게 확실한 상태"만 풀어 준다.
2. **본인 재사용 때도 `bound_percent` 를 덮어썼다.** 관리자가 요율을 내리면 이미 쓰던 고객도 깎인다
   (§2 스냅샷 약속 위반). → 본인 경로는 쓰기를 하지 않는다.

설계문서 §5-2 에 정정 박스를 달았다. 두 결함 모두 테스트로 고정했고, 원안대로 되돌리면 red.

### 같이 넣은 방어

- **코드 추측 시도 제한**: 계정당 KST 하루 **없는 코드 10회**. 056 `consume_member_benefit` 재사용(마이그레이션 없음).
  맞는 코드·등록된 본인 코드·형식 불통과 입력은 안 센다. 비로그인은 DB 조회 자체를 안 한다.
- **staging 가드**(설계 §6): 운영에서는 `batch='staging-test'` 거부, staging·로컬에서는 그 배치만 허용,
  그 밖의 호스트는 전부 거부(아래 리뷰 3번). 같은 DB 라 테스트가 실물 전단 코드를 귀속시키면 그 고객의 쿠폰이 탄다.
- **전(재화) 상품 제외**: `taste_dialogue_entry`(전 3개 지급)는 할인하지 않고 이유를 표시.

### 폴백 3곳 제거

`toss-membership-checkout.tsx` 의 `prepare.amount > 0 ? prepare.amount : amount` → 서버 금액이 없으면 결제창을 안 연다.
`checkout/page.tsx` 의 `?? paymentPackage.price` 2곳 → `quote.chargeAmount`. 셋 다 가드 테스트로 고정.

### 적대적 리뷰(3관점 + 반박 검증)에서 살아남은 3건 — 전부 반영

13건 중 10건은 반박됐다(중복·수용된 위험·PR4/PR6 범위). 남은 3건:

| # | 판정 | 문제 | 수정 |
|---|---|---|---|
| 1 | CONFIRMED | 시도 카운터가 **렌더 1 + prepare 1** 로 이중 차감 → 한도 직전 고객이 할인가를 본 뒤 결제 버튼에서 `rate_limited` 409 | **없는 코드만** 센다(읽기로 확인 → 조회 → not_found 일 때만 차감) |
| 2 | CONFIRMED | prepare 가 화면 금액을 몰라, 미리보기 뒤 요율이 바뀌면 **화면보다 비싸게** 청구되고 409 도 안 났다 | 체크아웃이 표시 금액(`expectedAmount`)을 보내고 prepare 가 **대조만** 한다 — 다르면 귀속 전에 409 + 새로고침 안내 |
| 3 | PLAUSIBLE | staging 가드를 호스트로만 판정 — `/api/*` 는 canonical 301 에서 빠져 프로덕션 배포에 `*.vercel.app` 으로도 닿고, 그 호스트는 'preview' 라 staging-test 코드가 실결제에 먹힐 수 있었다 | 3단계 판정: 운영=실물만 · staging/local=테스트만 · **preview/unknown=전부 거부**, `VERCEL_ENV=production` 이면 호스트와 무관하게 테스트 코드 거부 |

같이: 409 응답에 사유 코드(`couponRejected`)를 싣던 것을 뺐다 — 클라이언트는 안 쓰고, 화면이 일부러 뭉갠
구분(`env_mismatch`·`disabled`)을 API 가 알려 주는 오라클이었다. 사유는 퍼널(`prepare_blocked`)에만 남긴다.

2번은 쿠폰 전부터 있던 **가격 변경 레이스**(관리자가 가격을 바꾸는 순간 열려 있던 체크아웃)도 같이 닫는다.

### 검증

유닛 1,553 통과(+30) · typecheck · build · 빌드 산출물로 체크아웃 렌더 확인(무쿠폰·비로그인 경로).
**뮤테이션 13/13 red** — CAS 조건 제거·원안 상태목록·본인 경로 쓰기·폴백 복원 2곳·만료 재검사 제거·
staging 가드 제거·시도 제한 제거·회수 24h 재확인 제거·`coupon: null` 고정·리뷰 수정 3건 되돌림 각각을
넣으면 테스트가 깨지는 것을 확인했다.

### 남은 것 / 알려진 한계

- 🔴 **PR4(승인 직전 재검증)는 PR6(발급)보다 먼저 머지돼야 한다.** 지금은 할인가가 박힌 `prepared` 주문이
  쿠폰 회수·만료 뒤에도 승인될 수 있다(발급 0개라 현재 노출 0).
- 🟡 **결정 필요(리뷰어 2명이 독립적으로 지목)**: 등록된 쿠폰이 만료·회수돼도 그 행이 계정당 1개 자리를 계속 차지해
  새 코드를 못 쓴다(079 인덱스가 `where bound_user_id is not null` 뿐). 설계의 실무 대응 "소진된 배치를 끄고 재발행"과
  정면 충돌한다 — 끈 배치의 정상 고객은 재발행 코드를 영구히 못 쓴다. 요구 6 이 "동시에 1개"인지 "평생 1개"인지 확정 필요.
- 🟡 체크아웃 `?coupon=` 미리보기는 귀속 없이 "살아 있는 코드 + 할인율"을 알려 주는 오라클이다. 계정이 무료라
  계정당 한도로는 수확을 못 막는다. PR5(입력 UI)가 이걸 사용자 앞에 여는 PR 이므로 그 전에 전역 상한 여부를 정한다.
- 탈퇴하면 `payment_orders` 가 cascade 로 지워져, 그 사람이 쓰던 코드가 24시간 뒤 회수 가능해진다.
  원장 기반 판정의 한계 — 막으려면 승인 시 쿠폰 행에 사용 스탬프가 필요하다(PR4 에서 검토).

## 2026-09-11 — 할인쿠폰 PR2: `createPaymentOrder` 시그니처 교체 (기능 변화 0)

할인 계산을 **주문을 만드는 함수 안으로** 옮기고 `amount` 인자를 `listAmount`(정가) + `coupon` 으로
바꿨다. 쿠폰 로직은 아직 안 붙었다 — `coupon: null` 고정이라 동작은 종전과 완전히 같다.

### 왜 이름을 바꾸나

`amount` 라는 이름이 **존재하지 않게** 되므로, 미래에 새 결제 경로를 만드는 사람은 정가를 넘길
수밖에 없고 할인은 함수 안에서 자동으로 붙는다. **"쿠폰 적용을 잊는다"는 실수의 물리적 표현이 사라진다.**
실제로 기존 스냅샷 테스트가 컴파일 에러로 깨졌다 — 의도한 강제다.

### 타입이 못 잡는 자리 하나

`prepare` **응답**의 `amount` 는 타입 검사를 통과한 채 정가로 남을 수 있다. 그러면 클라이언트가
정가로 PG 청구 → `confirm:66` / `nicepay-return:216` 의 `order.amount !== amount` 에서
**쿠폰 없는 결제까지 전건 거부**된다(금액이 틀리는 게 아니라 매출이 0 이 되는 종류).

→ `amount: order.amount` 로 바꾸고 `resolvedAmount` 변수를 응답에서 참조하지 못하게 지웠다.
그리고 **가드 테스트로 고정**했다(`coupon-chokepoint.test.ts`) — 되돌려 red 나는 것 확인.

### 초크포인트 불변식 6개를 CI 로 고정

`payment_orders` insert 1곳 · `createPaymentOrder` 호출부 1곳 · PG 결제창 1곳 ·
prepare 응답 = `order.amount` · prepare 가 body 에서 할인율·금액을 안 읽음 ·
**가격 표시 맵에 user/coupon 의존 값 금지**(루트 레이아웃 전 방문자 공유 캐시라 넣는 순간 앱 전체가
동적 렌더가 되고 캐시가 폐기된다).

## 2026-09-11 — 할인쿠폰 PR1: 마이그레이션 079 + 순수 로직 (결정 4건 확정)

설계 #800 의 구현 1단계. **DB 에 쓰지 않고 돈에도 닿지 않는 기반만** 올린다.

### 사용자 결정 반영

| | 결정 | 반영 |
|---|---|---|
| A 일련번호 | **4자리**, 인쇄 `ganji-10-0000` / 저장 `ganji100000` | `parseCouponCode` 가 하이픈·공백·대소문자 흡수 |
| B 사용 조건 | **로그인이면 가능**(구글·카카오·이메일) | 신원 앵커 미도입 |
| C 인쇄 문구 | 미정(코드 형식만 확정) | — |
| D 만료일 | **2027-12-31**, 관리자 변경 | `expires_at` 기본값 |

### B 결정이 남기는 위험 — 기록해 둔다

`signup/route.ts:149` 가 `email_confirm: true` 라 **계정 생성 비용이 0**이다. "계정당 1개"는
손실 상한이 되지 못하고, 봇이 전단 코드를 귀속만 시켜 무력화할 수 있다.
남은 방어는 24시간 미결제 회수 · 시도 제한 · **배치 단위 `disabled_at` 회수** 셋뿐이고 전부 사후다.
→ 관리자 화면의 "배치별 귀속 속도"가 방어의 일부가 된다(귀속은 쏟아지는데 결제가 0 이면 사람이 개입).

### 돈에 닿는 두 규칙을 순수 함수로 고정

- **코드 접두 숫자는 할인율이 아니라 등급 라벨이다.** 코드에서 %를 계산하면 정본이 둘이 되어
  관리자 변경(요구 2)과 반드시 어긋난다. 파서는 등급만 뽑는다.
- **상한 50%는 관리자 입력 검증이 아니라 `applyCouponDiscount` 안에서 clamp 한다.**
  DB 직수정·마이그레이션 실수로 60이 들어와도 청구에는 못 넘어온다. 돈은 되돌릴 수 없다.

원 단위 절사(3,300×30% = 990 → 2,310), 청구액 하한 1원
(`payment_orders.amount CHECK (amount > 0)` 에 걸리면 prepare 가 500 이 되고 퍼널에 흔적도 안 남는다).

### 테스트를 쓰다 제 기대값이 틀렸다

`applyCouponDiscount(1000, 50, 999999)` 를 999 로 기대했는데 실제는 500 이다 —
`max_discount_won` 은 **상한**일 뿐 할인액을 늘리지 않는다. 코드가 맞고 테스트가 틀렸다.
금액 함수는 이런 방향 착각이 쉬우므로 상한/하한을 각각 독립 테스트로 남겼다.

⚠️ 마이그레이션 079 는 **프로덕션 수동 적용**(067·072·075 관례). 적용 전까지 쿠폰 기능은 죽어 있다.

## 2026-09-11 — 전(錢) 결제 환불이 **카탈로그 정가**로 나가고 있었다 (쿠폰 설계 검수 중 발견)

할인쿠폰 설계(#800) 적대적 검수 과정에서 드러난 **현존 버그**. 쿠폰과 무관하다.

`credit-refunds.ts:132` 는 주석에 "실결제액(metadata.amount) 우선 — 정가 우선이면 가격 개정 시
과거 결제가 소급 왜곡" 이라고 적어 두고, 실제로는 **항상 정가를 쓰고 있었다.**

```
fulfillment.ts:180·218  addCredits(..., { orderId, packageId, paymentKey })   ← amount 없음
        ↓
credit-refunds.ts:132   readNumber(row.metadata,'amount')  → 항상 null
                        ?? pkg?.price                       → 100% 이 폴백
```

### 왜 지금까지 안 보였나

**990원 결제 = 990원 정가라 우연히 같았다.** 전 결제 상품은 `taste_dialogue_entry` 하나뿐이고
`product_prices` 오버라이드가 걸린 적이 없어, 두 값이 한 번도 갈라지지 않았다.
갈라지는 순간은 셋이다 — `/admin/pricing` 가격 변경 · 할인쿠폰 · 이벤트가.

갈라지면 관리자 환불 화면이 정가를 표시하고, 부분환불이면 **실결제액보다 큰 취소액을 PG 에 쏜다**
(전액환불이면 `isFullRefund` 가 cancelAmount 를 안 실어 돈은 우연히 맞고 장부만 틀어진다).

### 수정

1. **신규 행**: `fulfillment.ts` 의 `addCredits` 두 곳에 `amount: claimed.amount` 를 싣는다.
2. **과거 행**: `buildCreditRefundItem` 에 `orderAmounts`(order_id→실결제액) 주입.
   호출부(`user-detail.ts`·`summary-refresh.ts`)가 **이미 읽어 둔** `payment_orders` 를 그대로 넘긴다.
3. 정가 폴백은 **삭제하지 않고 최후 수단으로 남긴다.** 그냥 지우면 metadata·주문 둘 다 없는 레거시 행이
   `null` → 환불액 0 → **환불 불가**가 되어, 2026-08-26 에 고친 "990원 냈는데 환불이 안 된다" 사고가
   그대로 재발한다. 순수 함수라 DB 조회를 안에서 할 수 없으므로 주입이 유일한 경로다.

### 검증

회귀 테스트 3개. 이 버그는 **정가와 금액이 다를 때만** 드러나므로 그 조건으로 고정했다 —
실결제 495원 / 정가 990원. 수정만 되돌려 `990 !== 495` 로 red 나는 것을 확인했다.

**교훈**: 폴백이 주 경로를 조용히 대체하고 있어도, 두 값이 우연히 같으면 영원히 안 보인다.
주석이 "A 우선"이라고 적혀 있으면 **A 가 실제로 채워지는지**를 같이 확인해야 한다.

## 2026-09-09 — "결제 전 확인" 글씨 되키움: 9px 는 너무 작았다

사용자: "결제전확인 폰트가 너무 작다 좀만 더 키워줘."

같은 날 9px 까지 줄인 걸 되올린다. **제목(17px)과 입력창(16px)은 그대로** 두고 본문만.
...
카카오 필드를 같이 올린 이유: 같은 카드 안인데 한쪽만 커지면 어색하다.

⚠️ 인라인 스타일이라 `text-[10`~`text-[15` 부양 패턴에 안 걸린다 — 클래스로 줬으면 11px 이
15px 로 튀었을 값이다([[project_unlayered-css-beats-tailwind]] 참조).

**손대지 않은 것**: 하단 고정바의 동의 문장(11px)·약관 링크(8.5px), '풀이 신뢰 안내'(9px).
요청이 "결제 전 확인" 카드였고, 고정바는 좁아서 커지면 결제 버튼을 밀어낸다.

## 2026-09-09 — 동의 체크박스를 하단 고정 결제바 안으로: 스크롤 왕복 제거

사용자 요청: "동의체크박스를 결제버튼 바로 아래로 옮겨서 바로 결제하기가 더 수월하게."

결제 버튼은 body portal 로 **뷰포트 하단에 고정**돼 있는데 동의 체크박스는 페이지 본문
위쪽에 있었다. 결제하려면 위로 스크롤 → 체크 → 다시 아래로 와야 했다.

### 버튼 **위**에 놓았다 (아래가 아니라)

요청은 "버튼 바로 아래"였지만 **버튼 위**로 넣었다. 전자상거래법상 동의는 결제 행위보다
앞서야 하고, 이 저장소가 2026-06-30 부터 그 화면 순서를 명시적으로 지켜왔다
(`toss-membership-checkout` 의 "결제수단·동의는 흐름상 위에 유지" 주석).

사용자가 원한 것(스크롤 왕복 없애기)은 위·아래 어느 쪽이든 똑같이 달성된다 —
같은 고정바 안이면 둘 다 한 화면에 있다. 그래서 **법적 순서를 지키는 쪽**을 택했다.

### 구조: 한 덩어리를 둘로 쪼개고 상태는 부모가

| 조각 | 내용 | 위치 |
|---|---|---|
| `PaymentConsentRow` | 체크박스 + 문장 + 약관 링크 4개 | **하단 고정바(버튼 위)** |
| `PaymentConsentDetails` | 제목 + 주문 요약 + 카카오 알림 필드 | 페이지 본문 |
| `usePaymentConsent(pkg)` | 동의 상태 단일 소유자 | 부모 |

두 조각이 같은 상태를 봐야 하므로 상태를 훅으로 올렸다. **덤으로 삭제된 것**: 기존
`onValidChangeRef` 우회 장치. 부모가 매 렌더 새 인라인 콜백을 넘겨 effect 가 재실행되고
무한 렌더("Maximum update depth exceeded")로 이어지던 걸 막던 코드인데, 상태를 부모가
소유하면서 원인 자체가 사라졌다.

⚠️ **법적 감사기록은 그대로다.** 체크 1회 → `acceptedKinds` 4종 전체 → prepare 가
`user_policy_consents` 4행 기록. 약관 링크 4개도 체크박스와 **같은 자리**에 유지했다
(통합 동의의 유효성 근거).
...
| | 결과 |
|---|---|
| 체크 전 | 버튼 disabled, "결제 전 동의가 필요합니다" |
| 체크 후 | 버튼 활성, "19,900원 카드로 결제하기" |
| 체크 해제 | 다시 disabled |
| 체크박스 위치 | 버튼 **위** |
| 고정바 높이 | 84px |
| 체크박스 개수 | **1개**(중복 렌더 없음) |

본문 섹션의 `aria-label` 도 `결제 전 동의` → `결제 전 확인` 으로 고쳤다 — 체크박스가 빠져
이름이 내용과 어긋났다.

### 후속 — 체크박스가 작아서 못 누른다 (사용자 신고)

**탭 영역은 글자 크기와 따로 논다.** 문장을 10px 로 줄이면서 체크박스도 15px 로 뒀더니
손가락으로 누르기 어려웠다. 글자를 줄이는 작업에서 놓치기 쉬운 지점이다.

- 체크박스 `15 → 20px` (+ `accent-[--app-pink]` 로 체크 시 브랜드 색)
- 문장 `10 → 11px`
- `label` 에 `min-height: 44px` — **행 전체(358×44px)가 탭 영역**이 된다.
  모바일 최소 터치 타깃 권장치(44px)를 행으로 확보.

실측: 문장을 눌러도, 글자 없는 아래 여백을 눌러도 토글된다. 고정바 84 → 113px.

**규칙**: 작은 글씨로 줄일 때 **입력 요소(체크박스·라디오·버튼)는 같이 줄이지 않는다.**
글자는 읽는 것이고 입력 요소는 누르는 것이라 기준이 다르다. 입력창 글자 하한 16px
(iOS 자동확대)과 같은 계열의 규칙이다.

## 2026-09-09 — 카카오 알림톡 필드 축소: 입력창 글자는 **줄이면 안 된다**(iOS 자동확대)

동의 블록을 9px 로 줄이자 그 아래 카카오 알림톡 필드가 상대적으로 커 보였다(사용자 요청으로
후속 축소).
...
| | 전 | 후 |
|---|---|---|
| 카카오 필드 박스 | 181px | **116px** (−36%) |
| 라벨 | 16px | **10px** |
| 설명 | 15px | **9px** |
| 입력창 글자 | 17px | 16px (유지) |
| 결제 전 확인 섹션 전체 | 412px | **347px** |

라벨이 16px·설명이 15px 로 렌더되고 있었다 — 코드엔 `text-[13.2px]`·`text-[12.1px]` 로 적혀
있었는데 **readability.css 가 전부 부양하고 있었다**(같은 날 앞 항목과 동일한 함정). 이번엔
처음부터 인라인으로 줬다.

### 입력창은 일부러 안 줄였다

`<input>` 글자를 **16px 미만으로 내리면 iOS Safari 가 포커스 시 화면을 자동 확대한다.**
확대된 뒤에는 되돌아오지 않아 결제 화면이 어긋난 채로 남는다. 글자는 그대로 두고 padding
(`py-2 → py-1.5`)·여백만 줄였다. 결과 16px 로 안전선 위에 있다.

**규칙**: 이 프로젝트에서 폼 입력창 글자는 **16px 이 하한선**이다. 미관상 더 줄이고 싶으면
글자가 아니라 높이·여백을 줄인다.

## 2026-09-09 — 결제 화면 축소: **작은 글씨는 지정해도 안 먹는다**(readability.css 가 되돌린다)

사용자 요청(스크린샷 2장에 빨간 동그라미): 동의 블록·신뢰안내 글씨를 아주 작게, 동의 문장과
약관 링크를 최소문장으로, 계좌이체 관련 문구 전부 제거.

### 처음엔 "줄였다"고 잘못 보고했다

`text-[10.5px]` 를 넣고 블록 높이가 줄어(865→656px) 적용된 줄 알았다. **폰트는 하나도 안
줄어 있었다** — 높이 감소는 padding·gap 변경분이었다. computed style 을 찍어보고 알았다:
클래스는 `text-[9px]` 인데 **실제 17px**.

### 범인: `readability.css`(@layer 밖)의 본문 최소 크기 부양

```css
.app-shell :where(p, li, dt, dd, label, legend, summary) { font-size: var(--app-text-body); }
.app-shell :where(.text-xs, [class*="text-[10"], [class*="text-[11"], [class*="text-[12"]) {
  font-size: var(--app-text-sm) !important;
}
```

두 갈래로 막힌다:
1. **태그 기반** — `p·li·dt·dd·label·legend·summary` 는 본문 크기로 되돌아간다.
   `:where` 라 특이도는 낮지만 **@layer 밖이라 Tailwind 유틸리티가 진다**(기존 기록:
   "레이어 밖 CSS가 Tailwind를 이긴다").
2. **클래스 패턴** — `text-[10`~`text-[15` 로 **시작**하는 클래스는 `!important` 로 부양된다.
   `text-[10.5px]` 도 `text-[11px]` 도 여기 걸린다.

→ 우회: **태그가 걸리면 인라인 스타일**(`style={{fontSize:'9px'}}`), **클래스 패턴이 걸리면
   패턴에 안 걸리는 값**(`text-[9px]`·`text-[9.5px]`·`text-[8.5px]`)을 쓴다.
   `span`·`a` 는 태그 목록에 없어 유틸리티가 그냥 먹는다.

**다음에 작은 글씨를 넣을 때**: 클래스만 넣고 끝내지 말고 `getComputedStyle` 로 실제 px 을
확인할 것. 이 저장소에서 작은 글씨는 기본적으로 **부양당한다**.

### 실측 (390px, computed 검증 완료)

| 블록 | 전 | 후 |
|---|---|---|
| 결제 전 확인(동의) | 865px | **449px** (−48%) |
| 풀이 신뢰 안내 | 293px | **103px** (−65%) |

computed: 신뢰안내 li 9px · 주문요약 li 9px · 동의 문장 9px · 약관 링크 8.5px · 제목 9.5px

### 문안 축약

- 동의 문장: "위 주문 내용을 확인하였으며, 이용약관, 개인정보처리방침, 환불·청약철회 정책,
  디지털 콘텐츠 제공·철회 안내 및 결제에 동의합니다." → **"주문 내용과 아래 약관을 확인했고
  결제에 동의합니다."**
- 약관 링크: "○○ 전문" → **"○○"** (링크 4개는 그대로)
- "동의 후 결제 버튼이 활성화됩니다. 동의 시점은 …(IP 원문은 저장하지 않습니다)" 3줄 **삭제**

⚠️ **줄인 건 문장뿐이다.** 동의 대상 4종 링크는 그대로 남는다 — 이 컴포넌트 상단 주석의 제약
("동의 대상을 화면에서 확인할 수 없으면 통합 동의의 유효성이 약해진다")을 지켰다.
서버 감사기록(`user_policy_consents` 4행)도 종전과 동일하다.

### 계좌이체 문구 제거 (화면 4곳)

흐름 안내 · 결제완료 수단표기 · 결제 버튼 하단 문장 · 미사용 상수(`CHECKOUT_METHODS`).
렌더한 페이지 전체에서 '계좌이체' 0건 확인. 남은 건 **주석과 토스 전용 옵션 라벨**뿐이다.

### 2차 수정 — 제목 원복 + 약관 링크 압축

사용자 지적: "결제 전 확인 타이틀은 줄이면 안 돼. 약관 링크들 간격과 넓이가 너무 벌어져 있어."

- 제목 `9.5px` → **`15px` 원복**(readability 부양으로 실제 17px = 종전과 동일)
- 약관 링크 `li` 에도 인라인 크기를 줬다. **글자만 8.5px 로 줄이고 `li` 를 방치하면 줄 상자가
  본문 크기(17px)로 남아 "글자는 작은데 간격만 벌어진" 상태가 된다** — 이게 벌어져 보인 진짜
  원인이다. `gap-x` 도 10px→6px, `gap-y` 0.
...
**교훈 보강**: 작은 글씨를 넣을 때 `a`·`span` 만 줄이면 **부모 `li`·`p` 의 줄 상자가 남는다.**
글자 크기와 줄 상자는 따로 논다 — 둘 다 확인할 것.

### 남은 것

동의 블록이 작아지니 그 아래 **카카오 알림톡 입력 필드가 상대적으로 커 보인다**(요청 범위
밖이라 손대지 않음). 균형이 어색하면 같이 줄일 것.

## 2026-09-08 — 결제수단 목록이 클라 env 하나에 매달려 있었다: provider 를 서버에서 내린다

`membership/checkout/page.tsx` 가 `TossMembershipCheckout` 에 **`provider` 를 안 넘기고** 있었다.
그러면 픽커가 `NEXT_PUBLIC_PAYMENT_PROVIDER`(빌드타임 인라인) 하나에만 의존한다.

### 실제로 재현된다

서버 `PAYMENT_PROVIDER=nicepay` + 클라 env 미설정으로 렌더:

| | 버튼 목록 | 계좌이체 |
|---|---|---|
| 수정 전 | 카드 · **실시간 계좌이체** | **노출** |
| 수정 후 | 카카오페이 · 네이버페이 · 삼성페이 · 카드 | 없음 |

즉 나이스페이로 결제하면서 화면엔 토스용 목록이 뜬다. 결제가 막히진 않는다
(`toNicepayMethod` 가 매핑 없는 코드를 `'card'` 로 떨어뜨리는 3번째 관문이 잡는다) —
증상은 "계좌이체를 눌렀는데 카드 창이 뜬다"는 혼란이다.

### 왜 놓쳤나

`NEXT_PUBLIC_*` 은 **빌드 시점에 문자열로 박힌다.** 서버 env 를 바꿔도 클라 번들은 안 바뀌고,
Vercel 에서 값을 넣어도 재배포 전엔 반영되지 않는다. 그런데 "결제 PG 전환은 `PAYMENT_PROVIDER`
한 줄"이라는 인식이 있어서, 클라 쪽 짝(`NEXT_PUBLIC_`)이 조용히 어긋날 수 있다는 걸 못 봤다.

→ **서버 env 가 정본, 클라 env 는 폴백.** 서버 컴포넌트가 `getPaymentProvider()` 로 읽어 prop 으로
   내린다. 클라 폴백은 픽커를 쓰는 다른 진입점(결제 전 클라이언트)용으로 남긴다.

**교훈**: `NEXT_PUBLIC_` 값으로 분기하는 화면은 서버가 같은 값을 알고 있다면 **prop 으로 내려라.**
빌드타임 인라인과 런타임 서버 env 는 언제든 어긋날 수 있고, 어긋나도 에러가 안 난다.

## 2026-09-08 — 간편결제 브랜드 버튼: 미선택을 opacity 로 죽이면 브랜드 컬러의 목적이 사라진다

사용자 요청: "카카오페이(노란색) 네이버페이(녹색) 삼성페이(파란색) 각 고유의 컬러버튼으로,
한 줄짜리로, 아래 설명은 빼줘."

- 2열 그리드 → **1열**, 버튼당 한 줄(로고 + 이름 + 선택 체크)
- 옵션별 설명 문구 제거, 상단 안내는 한 줄로 축약
- 공식 CI 컬러: 카카오 `#FEE500`(글자 `#191600`) · 네이버 `#03C75A` · 삼성 `#1428A0`
- 카드/계좌이체 등 브랜드 없는 수단은 기존 `gangi-payment-row` 유지

### 처음 만든 게 틀렸다 — 미선택을 `opacity-60` 으로 처리했더니

390px 실렌더로 보니 **카카오 노랑이 베이지, 네이버 초록이 민트, 삼성 남색이 보라**로 보였다.
사용자가 "색으로 찾게 한다"는 것이 이 버튼의 존재 이유인데 그 목적을 스스로 무너뜨린 셈이다.

→ 브랜드 컬러는 **항상 원색**. 선택 표시는 링(`ring-2 ring-[--app-ink]` + offset) + 체크로만.
   미선택에는 `ring-1 ring-black/10` 만 얹어 경계를 준다.

**교훈**: 브랜드 컬러에 opacity 를 씌우면 그건 더 이상 그 브랜드 컬러가 아니다. 상태 표현은
색을 흐리는 대신 **덧붙이는 요소**(링·체크·그림자)로 한다.

### ⚠️ 로고는 공식 에셋이 아니다

외부 이미지는 CSP 로 막혀 있고 각 사 로고 파일도 없어서, 형태를 본뜬 **인라인 SVG**를 넣었다
(카카오 말풍선 · 네이버 N · 삼성은 카드 글리프). 브랜드 가이드상 로고는 원본 사용이 원칙이므로
각 사에서 공식 결제버튼 에셋을 받으면 교체할 것. `BRAND_MARKS` 한 곳만 갈아끼우면 된다.

## 2026-09-08 — 간편결제 개별 버튼: 결제창 순서를 못 바꾸니 순서를 앱으로 끌어왔다

설계(#790) 구현. 나이스페이 결제창은 **수단 순서를 제어할 파라미터가 없다**(매뉴얼 UI 파라미터
7개 전수 확인 — disableScroll·skinType·logoImgUrl 등뿐). 그래서 우리 화면에서 수단을 직접
고르게 하고, 고른 수단의 창이 바로 열리게 했다.

### 화면 (나이스페이, 390px 실렌더 확인)

```
카카오페이       → kakaopay        (카드/머니 둘 다 — 카카오 창에서 사용자가 선택)
네이버페이       → naverpayCard    (⚠️ 매뉴얼상 포인트 이용불가)
삼성페이         → samsungpayCard
신용/체크카드    → card            ← 기본 선택
```

**배열 순서가 곧 화면 순서다.** `methods.ts` 의 `TOSS_PAYMENT_METHOD_OPTIONS` 하나가 정본이고,
순서·라벨·나이스페이 매핑이 한 곳에 모인다. 순서를 의도 없이 못 바꾸게 테스트로 고정했다.

페이코·SSG 는 계약 미개설이라 넣지 않았다(열리면 2줄 추가).

### 삼중 안전장치 — 계좌이체가 새면 결제가 막힌다

1. 테이블: `TRANSFER.nicepayMethod = null`
2. 목록: `paymentMethodOptionsFor('nicepay')` 가 `nicepayMethod === null` 을 제외
3. 매핑: `toNicepayMethod` 가 매핑 없는 코드를 `'card'` 로 떨어뜨림(안전한 실패)

`TRANSFER`·`bank`·`vbank`·`cellphone`·빈 문자열·`unknown` 어느 것도 `'bank'` 로 안 나간다는 걸
테스트로 고정.

### 토스 경로는 그대로

`paymentMethodOptionsFor('toss')` 는 `['CARD','TRANSFER']` — 간편결제 코드는 토스로 **한 개도**
안 간다(토스는 그 코드를 모른다 → 결제가 깨진다). provider 미지정도 토스 취급이다.
390px 에서 양쪽 다 렌더해 확인했다.

### CARD 의 의미가 하루 만에 두 번 바뀌었다

`card`(원래) → `cardAndEasyPay`(#788) → `card`(지금). #788 은 "카드 창 안에 간편결제도 같이"
였고, 지금은 간편결제를 **개별 버튼으로 분리**했으니 카드 창에 또 넣으면 같은 수단이 두 번
나온다. #788 이 넣은 고정 테스트가 이 변경을 정확히 막아섰다 — 의도된 브레이크이고, 그게
그 테스트의 일이었다.

## 2026-09-08 — 실시간 계좌이체 영구 제외: "켜면 안 되는 플래그"를 남겨두는 것 자체가 지뢰다

사용자 지적: "실시간 계좌이체는 빼줘. 저건 나이스페이에서 넣는 순간 결제 막아버린다고 해서
넣으면 안 돼."

2026-06-27 엔 W004(발급 clientId 에 미연결)로 **임시 숨김**이었고, 되살릴 스위치로
`NEXT_PUBLIC_NICEPAY_TRANSFER_ENABLED=true` 를 남겨뒀다. 전제가 바뀌었다 —
"키를 연결하면 복구"가 아니라 **애초에 넣으면 안 되는 수단**이다.

그 스위치는 **env 한 줄로 라이브 결제를 죽이는 지뢰**다. 삭제한다.

### 2중으로 막았다

1. **픽커**: `hideTransfer = provider === 'nicepay'` — env 조건 삭제, 무조건 제외.
2. **매핑**: `toNicepayMethod` 의 `'TRANSFER' → 'bank'` 삭제. 입력이 무엇이든
   `cardAndEasyPay` 로 떨어진다(안전한 실패). 여기가 `bank` 가 새는 마지막 관문이라
   "어떤 입력에도 bank 로 매핑되지 않는다"를 테스트로 고정했다.

UI 를 우회해 TRANSFER 가 흘러들어와도 결제창에는 `bank` 가 안 나간다.

### 교훈

"임시 숨김 + 복구 플래그"는 **복구가 실제로 가능할 때만** 안전하다. 복구하면 안 되는 것에
플래그를 달아두면, 나중에 사정을 모르는 사람(또는 미래의 나)이 켠다. 되살리면 안 되는 건
플래그가 아니라 코드에서 지우고, 이유를 그 자리에 적는다.
...
`GangiServiceCardLink` 의 wide 배너(`gangi-market.tsx`). #779 에서 "좁은 화면에선 가격을
아랫줄로" 하려고 가격 묶음에 `w-full sm:w-auto` 를 줬는데, **부모 행에 `flex-wrap` 을 안 넣었다**
(#779 가 고친 다른 3곳은 전부 wrap 을 같이 넣었다 — 여기만 빠졌다). wrap 없는 flex 행에서
`w-full`+`shrink-0` 형제는 줄을 바꾸지 않고 행 전체를 먹고, 남은 텍스트 블록(`min-w-0 flex-1`)이
0 폭으로 찌그러진다. 그래서 "1:1 대화 / 대화상담 / 선생님께 질문 3회" 가 두 글자만 남고 사라졌다.

수정은 부모에 `flex-wrap` 한 단어. 390px 에서 before/after 렌더해 확인했다(before: "1:1 / 대화"
만 보임, after: 제목·설명 전부 + 가격칩 아랫줄).

교훈: `w-full` 로 "아랫줄로 내리는" 처리는 **부모의 `flex-wrap` 과 한 세트**다. 한쪽만 있으면
줄바꿈이 아니라 형제 압살이 된다.

## 2026-09-04 — "며칠째 가입·결제 0" 진단: 코드가 아니라 **광고 유입이 8/28에 끊겼다**

사용자 신고: "몇 일 동안 신규 가입자도 결제하는 사람도 한 명도 없다. 유입은 계속 있는데."
프로덕션 읽기 전용 집계(사용자 승인)로 원인을 확정했다. **코드 변경 없음 — 이 기록이 산출물이다.**

### 처음 세운 가설이 전부 틀렸다

`payment_orders` 가 9/1 이후 0행, `prepare_attempt`(prepare 라우트 진입 즉시 기록) 도 8/31 22:06
이후 0. 반면 `paywall_viewed` 는 하루 271~648, `readings` 는 하루 89~131 로 정상. 이 조합은
"결제 버튼 클릭 → `/api/payments/prepare` 호출" 구간만 죽은 것처럼 보인다. 실제로 그 구간엔
**흔적 없이 죽는 경로가 3개**나 있어서(아래) 회귀를 강하게 의심했다.

- `toss-membership-checkout.tsx` 의 `handlePayment()` 조기 return 3곳은 **퍼널에도 주문에도 안 남는다**:
  ① `requiresSlug` 상품인데 slug 없음 ② 미로그인(→ `login_required` 만 기록) ③ `NEXT_PUBLIC_TOSS_CLIENT_KEY` 부재
- 타이밍도 그럴듯했다: 마지막 결제 시도(8/31 22:06) **직후** 아이콘 전면 리브랜딩이 배포됐고
  (#747~#754), 그중 #754 는 "InkIcon 이 모르는 이름을 삼켜 하단 메뉴 아이콘이 사라진" 사고다.

**전부 틀렸다.** 사용자가 프로덕션에서 직접 결제 버튼을 눌러 확인:
```
23:14:58  prepare_attempt   taste_dialogue_entry  (logged)
23:14:59  prepare_ready
23:14:58  payment_orders 1행 (prepared / 990 / CARD)
```
결제창 정상 오픈, 계측 정상 기록. → **아무도 누르지 않은 게 사실**이었다.

### 진짜 원인 — 유입원 일별 분해

`site_visits.referrer_host` 를 일별로 세자 한 줄로 드러났다.

| 날짜 | 방문 | facebook/instagram | link.inpock(프로필링크) | 주문 |
|---|---|---|---|---|
| 08-27 | 356 | **192** | 93 | 4 |
| 08-28 | 288 | **147** | 87 | 7 |
| 08-29 | 133 | 29 | 63 | 0 |
...
**8/28 을 기점으로 페이스북·인스타 유입이 147 → 1 로 죽었다.** 총 방문 356→96(-73%)의 감소분이
사실상 전부 이 채널이다. 남은 유입은 인포크링크(인스타 프로필 링크) = 기존 팔로워 재방문이라
구매 전환이 가장 낮은 트래픽이다. 결제가 난 날(8/24~8/28, 8/31)은 전부 facebook 유입이 살아
있던 날이다. 끊긴 시점은 9/1 이 아니라 **8/28** 이고, 8/31 의 4건은 그 잔여였다.

사이트 내부는 멀쩡하다: 결과 생성 89~131/일, 3PV 이상 심층 방문 30~61명, 카카오 로그인 정상
(9/3·9/4 에도 로그인 발생), 신규 가입도 0 이 아니다(9/1 3명·9/4 1명 — 원래 하루 0~3명이고
**가입은 결제 직전에만 필요**하므로 결제가 0 이라 가입이 0 인 것이지 그 반대가 아니다).

### 다음에 같은 신고를 받으면 (순서를 지켜라)

1. **먼저 유입원을 일별로 쪼갠다.** `site_visits.referrer_host` 한 방이면 채널 붕괴가 보인다.
   "유입은 있는데"는 **총량**만 본 진술이다 — 총량이 유지돼도 채널이 바뀌면 매출은 0 이 된다.
2. 그 다음 계측 신뢰성. `paywall_viewed`(하루 271~648)는 결과 화면 1 개에 상품 2 개가 각각 찍히고
   새로고침마다 누적돼 **방문자 대비 4~5 배로 부푼다**. 분모로 쓰면 전환율을 오독한다.
   `checkout_viewed` 가 동일 초에 tarot-daily+dialogue-hub 쌍으로 찍히면 사람이 아니라 봇이다.
3. 코드 회귀 의심은 **마지막**에. 확인은 추론이 아니라 사용자가 실제로 한 번 누르는 것이다
   (여기선 그 한 번이 3 개 가설을 동시에 기각했다).

⚠️ 이 진단이 무너지는 조건: 메타 광고가 8/28 이후에도 정상 집행 중이고 노출·클릭이 그대로라면
틀렸다. 그땐 광고 클릭이 사이트에 도달하지 못하는 문제(링크 만료·리다이렉트)를 봐야 한다.
DB 로는 광고 계정 상태(중단/예산소진/소재반려)를 볼 수 없다 — 메타 광고 관리자에서 확인이 필요하다.
...
**머지 전 리뷰에서 잡은 것**: 경로 기록을 처음엔 `void service.rpc(...)` 로 띄웠는데,
**서버리스는 응답 직후 함수를 얼려 그 기록이 조용히 유실된다.** `after()` 로 옮겼다 —
응답은 그대로 안 늦고 실행은 보장된다. 가드도 "await 금지"에서 "after 안에서 실행"으로 정정했다.

**같이 고친 것 — main 이 이미 빨간 상태였다.** #773 이 넣은 퍼널 라벨 `로그인 후 복귀` 를
`audit:mockup-placeholders:strict` 가 "하드코딩 안내"로 오탐해 CI 가 죽어 있었다(#773 머지 후
main 에서 발생). 퍼널 **단계 이름**이라 분기 대상이 아니므로 문서화된 마커
`{/* audit-mockup: intentional */}` 를 그 줄 바로 위에 달았다(마커는 같은 줄 또는 직전 줄만 인식).
